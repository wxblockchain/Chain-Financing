...
## 6.1 通知组件（C-20～C-23，双端复用）

实现在 `_shared/`（`base.css` 的 `.bell` / `.dd-list.panel` / `.msg*`，`shell.js` 的铃铛、面板与 `CF.msgItem`）。
**它们是壳层能力，不是某个模块的私有组件**：任何模块都不得在自己的文件里另画一套。

| 组件 | anatomy | 组合规则 | 禁止用法 |
| --- | --- | --- | --- |
| C-20 通知铃铛 + 未读角标 | 圆形图标按钮 + 右上角实心角标；1 位数圆形、2 位及以上胶囊 | 固定在顶栏语言切换器**左侧**、账号下拉之前，双端同一位置；数值上限外展示 `99+`；变化通过 `aria-live="polite"` 播报；没有消息数据源的模块只渲染入口、不渲染角标 | 无未读时展示 `0` 或空角标；裁切角标数字；按分类拆成多个角标；只用红点不给数值可读名称 |
| C-21 快捷面板 | 铃铛下方的受控浮层：标题行 + 列表区 + 底部单一「查看全部」 | 每次展开重新拉取，先给骨架屏；空态与失败态在面板内解决；Esc / 点击面板外收起并把焦点还给铃铛 | 在面板里放「全部已读」这类影响面广且不可撤销的操作；把面板做成第二套消息中心；面板内维护自己的一份已读状态 |
| C-22 消息列表项 | 未读点 + 分类标签 + 标题 + （进度型的节点行）+ 摘要 + 时间 + 行内次操作 | 通用型与进度型是同一组件的两个变体；标题与摘要各截断一行；行内「标为已读」在精确指针下悬浮出现，键盘 focus 与触控设备下常驻 | 用加粗以外的手段区分未读；把已读项从列表中移除或重排；只用颜色表达进度状态而不带文本 |
| C-23 失效说明条 | 详情正文上方的浅底说明条，左侧信息图标，纯文本不可关闭（复用 `.note` 的信息变体） | 四种失效原因四套文案，逐条说明「为什么不能跳」 | 使用错误红色（失效是正常业务状态）；四种原因合并成一句「该消息已失效」；隐藏消息内容 |

**跳转按钮的两种「没有」必须区分**：目标存在但当前不可达 → 按钮**禁用**并给原因（C-23 说明条）；
没有可用目标（本来就没有 / 客户端未登记该业务类型 / 消息已过期）→ **不渲染**按钮，
且**界面上不给任何解释文案**，只保留返回入口（用户 2026-09-08 裁定）。
前者要解释「为什么点不了」，后者不需要解释——用户看不到按钮就不会去找原因，
写一句「当前版本暂不支持跳转」反而在告诉用户系统缺了点什么。
技术侧的降级原因（未登记的业务类型、缺失的文案模板、预检超时放行）走日志与规格文档，不上界面。
