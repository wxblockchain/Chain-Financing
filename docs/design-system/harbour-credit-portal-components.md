...
现有实现：`asset-platform/prototypes/账户与登录/v1.0-账户与登录-原型.html` 的资产端部分（总览、用户中心、账户设置）与 focus 版式（登录、重置密码）；**顶栏版式的首个落库实现是 `financial-service-platform/prototypes/金融服务端登录/`**（实现层为 `_shared/base.css` 的 `.portal*`，见[现有页面继承参考 4.3](harbour-credit-page-prototype-reference.md)）。公开层自己的页面尚未落库，接入时继承同一根顶栏。

面客端的核心特征是：同一浅色金融科技 token 基础、居中宽内容容器、轻量顶栏、较舒展的留白、卡片与摘要信息并置，以及对结构化短值的谨慎等宽展示。

## 2. Token 与排版

沿用 [token 总览](README.md#token-总览)。面客端的额外规则如下：
...
| 元素 | 布局规则 | 禁止用法 |
| --- | --- | --- |
| 顶栏 | sticky 白色表面、1px 下边线；品牌、主导航、语言/上下文操作分区。实现为 `.portal-head`，见[继承参考 4.3](harbour-credit-page-prototype-reference.md) | 引入管理端左侧导航或内部工具栏密度；在同一页里再放第二套导航；把账户级页面塞进主导航 |
| 主导航 | `.portal-nav` 单行、不换行；项目由登记表渲染，模块不自建菜单；容器放不下时整条横向滚动，≤768px 顶栏拆成品牌 / 主导航 / 工具区三行 | 让导航标签在窄屏换成两行；把一条流程的中间页登记成顶级导航项 |
...
| Code / hash token | `--mono`、`accent-soft`、`accent-border`、7px radius | 用于短结构化值；长字符串提供完整值可达性 |
...
与管理端**同一套实现、同一套语义**，详见[管理端组件规范 6.1](harbour-credit-admin-components.md#61-通知组件c-20c-23双端复用)。
资产端只有两点差异，且都不改组件本身：

- 密度由 `[data-end="asset"]` 的控件高与卡片内边距承担，铃铛与面板不另设一套尺寸；
- 消息列表的分页在资产端是「加载更多」按钮而不是分页器——消息的浏览行为是从新往旧扫一遍，
  分页器会强迫用户记住自己看到第几页，而无限滚动在「进详情再返回」时位置恢复不可靠。
