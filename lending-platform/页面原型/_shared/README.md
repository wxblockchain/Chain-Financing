...
**页面接入必读：** [面客工作区组件规范 §0：阅读顺序、样板版本、逐页继承](design-system/面客工作区组件规范.md#0-必读入口顺序与样板版本) → [§5.1：内嵌布局与加载策略](design-system/面客工作区组件规范.md#51-内嵌布局与数据加载必须分别决定) → [§8：浏览器验收](design-system/面客工作区组件规范.md#8-响应式验收)。不能仅按 PRD 的分页要求省略样板内嵌容器。
