...
## 面客端共享账户入口（2026-09-21）

已登录顶栏按「消息 → 语言 → 钱包地址」排列，地址显示前 6 位和后 4 位（含 `0x`），完整值保留在入口 title 与无障碍名称中。角色只在账户悬浮菜单首行显示；账户设置、资料与退出沿用原入口。

`CF.portalAccount()` 返回 `{walletAddress}`，由登录宿主提供当前登录账号地址：资产方与账户详情复用同一数据，资金方读取签名后登录账号，而非当前连接的插件地址。宿主返回空值时显示「账户」，不编造地址。未接入登录宿主的独立原型沿用 `0x1111…1111` 演示身份；这只是展示样例，不代表真实钱包或跨模块会话。游客仍显示登录入口。共享层修改后须重新导出原型，历史附件不会自动更新。
