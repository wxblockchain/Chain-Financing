...
`CF.moreFoot` 提供尾部外观。需要异步失败/重试的业务模块使用自己命名的动作（资产广场 `am-more`）与请求状态，不交给样板的同步 `more` 哨兵；已有记录和汇总在追加时保持。恢复必须先按 URL 重建已加载范围，再恢复位置，不能只读取本地会话缓存。详细契约见[组件规范 §5.1](design-system/面客工作区组件规范.md#51-内嵌布局与数据加载必须分别决定)。
