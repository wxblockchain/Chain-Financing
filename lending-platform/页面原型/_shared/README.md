...
详情与流程页面在 `CF.PAGES[pageId].parent` 声明真实父页面 ID；不登记成新增菜单。面客端只沿真实父级链渲染：资产广场、借贷广场、我的控制台、消息中心和账户设置等一级入口仅展示当前名称，不在前方补资产广场或其他入口；详情仍显示「所属列表 › 当前页」。管理端沿用可用模块根页。使用明确路由，直接打开深链也能返回。已有查询参数优先沿用 `CF.ENTRY`，需要记录 ID 或视图参数时由 `breadcrumbRoute(pageId)` 返回。返回继续经过模块既有路由守卫，不调用 `history.back()`；模块不得重绘公共面包屑。
