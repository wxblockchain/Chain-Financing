...
**原型评审切换统一规则（WS-385）：** [先定登录状态，再选页面与该页状态](design-system/原型评审切换规范.md)。登录状态（面客端）、页面目录、联动状态与恢复默认由共享运行时提供，评审工具默认关闭；模块复用原业务流程与权限保护。
...
| 滑块（拼图）人机校验（两端唯一实现） | [`puzzle.js`](puzzle.js) + `base.css` 的 `.cf-puzzle` |
...
## 滑块人机校验（2026-09-24 · WS-380 收敛）

`puzzle.js` 是两端唯一实现，模块不再自持第二份副本；运营端账户与登录原来的自有拼图已并入本组件，面客端资金方账户的既有调用不变。

宿主负责承载弹层、取消出口与通过之后要做的事，组件负责题目、对齐校验、失败换题与冷却：

| 接口 | 说明 |
| --- | --- |
| `configure({store,hintAfter,coolAfter,coolMs,help,guard})` | 可选。`store` 为冷却存储键，省略时仅存内存；`hintAfter` 为提示可换题的连续失败次数；`coolAfter` / `coolMs` 为进入冷却的次数与时长（默认 5 次 / 60 秒）；`help` 为求助文案；`guard` 判断宿主承载是否仍在 |
| `start(onPass)` | 发起一次新校验，通过后回调宿主。上一次通过不能复用 |
| `stop()` / `active()` / `status()` / `wait()` | 结束、是否进行中、当前状态、冷却剩余秒数 |
| `body()` | 返回校验区 HTML，放进宿主自己的弹层 |
| `failNextLoad()` / `reset()` | 演示工具：模拟一次加载失败、清空本演示环境的失败次数与冷却 |

换题与求助按钮由组件自己处理，宿主不必接线。配置 `store` 后冷却与连续失败次数按绝对时间持久化，刷新、换入口或另开标签页都不能绕过，同一浏览器的其他标签页进入冷却时本页立即跟随。题形、答案与判定只用于本地演示，不具备真实防自动化能力；服务端挑战、票据防重放与真实限频仍须后端实现。发码票据（目标邮箱、用途、来源与有效期）属业务规则，留在各自模块。

## 面客端共享账户入口（2026-09-21）

已登录顶栏按「消息 → 语言 → 钱包地址」排列，地址显示前 6 位和后 4 位（含 `0x`），完整值保留在入口 title 与无障碍名称中。角色只在账户悬浮菜单首行显示，菜单内不展示钱包地址；账户设置、资料、企业账户与退出沿用原入口。登录未绑定角色（`S.role === "signed"`）只保留地址标识与退出，不显示铃铛、角色行、账户设置、资料入口与企业账户；需登录的导航入口对该状态同样隐藏。
...
菜单条目的 `permission` 可写成数组：其中任一项获准，入口即可达（协议管理为 `[9,16]`——只有操作记录权限的运营专员仍须能到达记录入口）；进入后读取哪些对象仍由各业务模块按目标权限判定。
...
| `demoOnly` + `demo()` | 可选：模块仅提供自己的扩展模拟内容，由共享层收纳至“更多模拟”；外层页面与状态选择统一由 `CF.review` 提供 |
...
详情与流程页面在 `CF.PAGES[pageId].parent` 声明真实父页面 ID；不登记成新增菜单。面客端与管理端均只沿真实父级链渲染。一级入口（包括各列表、总览、消息中心和账户设置）不展示面包屑，也不保留空行、边线或容器占位；详情显示「所属列表 › 当前页」，不补默认落地页作为祖先。使用明确路由，直接打开深链也能返回。已有查询参数优先沿用 `CF.ENTRY`，需要记录 ID 或视图参数时由 `breadcrumbRoute(pageId)` 返回。返回继续经过模块既有路由守卫，不调用 `history.back()`；模块不得重绘公共面包屑。

祖先使用可聚焦链接，当前页使用 `aria-current="page"` 非链接文字；分隔符对读屏隐藏。窄屏只收起较远祖先，保留直接上级与当前页，长标题可换行。面客端通过面包屑返回所属列表，不再附加同义返回按钮；取消、关闭及跨模块返回来源记录按原语义保留。管理端挂在页面内容区 `#acontent` 内、标题上方，与正文左边界对齐并随内容滚动；顶部工具栏只承载全局工具。公共层在内容重绘后维护 `#acrumb`，模块不自行移动或重绘。历史 site 骨架及居中登录页不增加面包屑；当前产品已移除首页，资产广场仅为默认落地入口，旧根地址兼容跳转。

## 资产广场连续追加接入

本模块已确认布局、默认入口及返回约定见[组件规范 §5.4](design-system/面客工作区组件规范.md#54-资产广场已确认组合与返回契约)。资产详情保留整宽纵向分区，通过面包屑返回原列表；共享详情组合仅为可选项，不能覆盖这些确认结果。
...
`portal-entry.js` 是当前五个面客 HTML 的唯一 JS/CSS 加载清单；`portal-init.js` 延后启动，`portal-connect.js` 在资产、L5–L8、控制台、账户、消息模块装配后统一启动。源码以顺序脚本加载，`export.py` 读取同一清单内联；管理端的文档切换机制不变。

组合层保留各模块已确认的内容与业务弹层，衔接账户/消息渲染钩子、控制台守卫、消息→项目→原消息、登录后的合法站内回跳。仅原型会话内共享 `CF.S` 及业务对象；持久化仍沿用各模块现有规则，不代表后端统一认证或真实事件投递已完成。消息的未知/不可见项目不会静默替换成另一条项目。
