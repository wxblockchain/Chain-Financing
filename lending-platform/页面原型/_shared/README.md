...
祖先使用可聚焦链接，当前页使用 `aria-current="page"` 非链接文字；分隔符对读屏隐藏。窄屏只收起较远祖先，保留直接上级与当前页，长标题可换行。面包屑已承担返回导航，不再附加「返回上级」按钮。管理端挂在页面内容区 `#acontent` 内、标题上方，与正文左边界对齐并随内容滚动；顶部工具栏只承载全局工具。公共层在内容重绘后维护 `#acrumb`，模块不自行移动或重绘。首页及居中登录页不增加面包屑。
