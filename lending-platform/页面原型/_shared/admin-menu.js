...
(function(CF){
  ...
  /* reachable：交付里勾了该项但缺配套查询权限时，入口仍须可达，让账号读到拒绝原因而不是静默消失。 */
  function reach(e){return can(e.permission)||(e.reachable||[]).some(n=>!!CF.opsAuth&&CF.opsAuth.held(n))}
  function allowed(e){if(!reach(e))return false;if(CF.opsAuth)return true;return !local(e)||!module?.allowNav||module.allowNav(e.page)}
  /* pages：同一入口体系下的多个页面共用一个侧栏条目的选中态。 */
  function active(e){let id=CF.S.page;while(id){if(id===e.page||(e.pages||[]).includes(id))return true;id=CF.PAGES[id]?.parent}return false}
