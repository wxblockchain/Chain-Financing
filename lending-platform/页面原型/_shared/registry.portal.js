...
(function (CF) {
  ...
  var pages = {
    ...
    /* 资产广场（已上架代币只读域）。
       P-AM-01 是原型侧暂定号：该模块的 PRD 页面编号由 WS-350 定，正式登记后回写，
       在那之前不要据此当成 PRD 已定义的页面编号。 */
    "P-AM-01": { layout: "portal", navKey: "navAssets", route: "/assets" },
  ...
  for (var id in pages) {
    var p = pages[id];
    CF.PAGES[id] = { end: "asset", layout: p.layout, navKey: p.navKey, auth: !!p.auth };
    CF.ENTRY[id] = p.route;
  }

  /* 顶栏主导航四项。账户设置与机构信息挂账号下拉，不占主导航位；
     流程页与详情页由列表或深链带出，同样不占导航位。 */
  CF.NAV.asset = ["P-F51", "P-AM-01", "P-LS-01", "P-MC-01"];
})(window.CF = window.CF || {});
