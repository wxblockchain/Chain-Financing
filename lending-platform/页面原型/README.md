...
**原型评审切换统一规则（WS-385）：** [先定登录状态，再选页面与该页状态](_shared/design-system/原型评审切换规范.md)。登录状态（面客端）、页面目录、联动状态与恢复默认由共享运行时提供，评审工具默认关闭；模块复用原业务流程与权限保护。
...
- 双击任一模块目录下的 HTML 即可离线评审，无需本地服务；请保持 `页面原型/` 目录完整。
- 样式与运行时在 `_shared/` 下，**把单个 HTML 复制或下载到别处打不开**，页面会给出提示并说明该从哪里打开。
- 管理端各模块是独立文件，侧边栏跨模块跳转按整页加载：演示身份与临时状态会重新初始化，「总览／账户设置」按模块 README 的演示凭据重新登录。
- 需要单独发给他人评审时，用 `_shared/export.py` 导出自包含单文件，不把生成件放回仓库。
- 区块浏览器等外部链接指向站外，离线状态下打不开属正常。
