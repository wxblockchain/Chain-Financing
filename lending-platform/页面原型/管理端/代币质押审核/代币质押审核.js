(function(CF){
  'use strict';
  const S=CF.S,L=CF.L,E=CF.esc,Q='P-O-PR-01',D='P-O-PR-02',ROOT='/ops/pledge-reviews';
  const status={pending:['Pending review','待审核','warn'],approved:['Approved','通过','ok'],rejected:['Rejected','驳回','danger'],withdrawn:['Withdrawn','已撤回','gray'],timeout:['Pledge failed · review timeout','质押失败（审核超时）','danger']};
  const follows={none:['Not initiated','未发起'],ready:['Awaiting deposit confirmation','待确认入池'],invalid:['Pre-check invalidated','复核作废'],expired:['Expired before initiation','未发起已过有效期'],processing:['Executing','执行中'],success:['Execution succeeded','执行成功'],failed:['Execution failed','执行失败'],verify:['Result under verification','结果待核实'],difference:['Reconciliation discrepancy','对账差异']};
  const categories=[['Ownership or entity in question','代币归属或主体存疑'],['Underlying receivable issue','底层应收账款存在问题'],['Token does not match project','代币与目标项目不匹配'],['Incomplete or inconsistent materials','申请材料不完整或不一致'],['Other','其他']];
  const fDefault=()=>({status:'pending',owner:'',project:'',type:'',from:'',to:'',q:'',follow:''});
  ...
  const A={rows:seed.rows,now:seed.now,clock:Date.now(),filter:fDefault(),editFilter:fDefault(),page:1,sort:'submitted',dir:1,
    filtersOpen:false,tab:'overview',role:'review',response:'success',upload:'success',preview:'success',activityState:'default',view:'default',draft:null,busy:false,error:'',selected:null,events:[],epoch:0};
  ...
  const canRead=()=>['review','read'].includes(A.role)&&A.view!=='denied';
  const row=()=>A.rows.find(r=>r.id===A.selected);
  const canWrite=r=>canRead()&&A.role==='review'&&r?.status==='pending'&&now()<r.deadline&&A.uncertain?.id!==r.id;
  ...
  function event(r,title,text='',actor=['System','系统'],extra={}){r.events.unshift({id:crypto.randomUUID(),at:now(),title,text,actor,...extra});}
  ...
  function syncContext(){
    ...
    A.selected=p.get('id');A.tab=p.get('tab')==='related'?'related':'overview';A.jumpActivity=p.get('tab')==='activity'||p.get('section')==='activity';
  ...
  function restoreView(){
    if(A.jumpActivity&&S.page===D&&A.tab==='overview'){A.jumpActivity=false;requestAnimationFrame(()=>{const el=document.getElementById('pr-activity');el?.scrollIntoView({block:'start'});el?.focus({preventScroll:true});});restoreContext=false;return;}
  ...
  function filtered(ignoreState=false){const f=A.filter;return A.rows.filter(r=>(!f.owner||r.owner===f.owner)&&(!f.project||r.project===f.project)&&(!f.type||r.type===f.type)&&(!f.from||r.submitted>=Date.parse(f.from+'T00:00:00'))&&(!f.to||r.submitted<new Date(new Date(f.to+'T00:00:00').setDate(new Date(f.to+'T00:00:00').getDate()+1)).getTime())&&(!f.q||[r.id,r.batch,r.token,owner(r)].join(' ').toLowerCase().includes(f.q.trim().toLowerCase()))&&(ignoreState||((!f.status||r.status===f.status)&&(!f.follow||r.follow===f.follow))));}
  function filters(){const f=A.editFilter,applied=A.filter;
    const extra=['owner','project','type','from','to','follow'],count=extra.filter(k=>applied[k]).length;
    const labels={owner:L('Asset holder','资产方'),project:L('Project','项目'),type:L('Application type','申请类型'),from:L('From','起始日期'),to:L('Through','结束日期'),follow:L('Follow-up','后续情况')};
    const values={owner:applied.owner?owner({owner:applied.owner}):'',project:applied.project?project({project:applied.project}):'',type:applied.type==='first'?L('Initial pledge','建池首笔'):L('Additional pledge','追加'),follow:applied.follow?txt(follows[applied.follow]):''};
    return `<form id="pr-filter-form" class="pr-filter-wrap"><div class="pr-filter-bar">${field(L('Search applications','搜索申请'),`<input class="inp" id="pr-f-q" value="${E(f.q)}" placeholder="${L('Application, token, batch or company','单笔、代币、批次或企业')}">`)}${field(L('Review status','审核状态'),select('pr-f-status',f.status,[['',L('All','全部')],...Object.entries(status).map(([k,v])=>[k,txt(v)])]))}<div class="pr-actions"><button type="button" class="btn" data-act="pr-filters" aria-expanded="${A.filtersOpen}" aria-controls="pr-advanced">${L('Filters','筛选条件')}${count?' ('+count+')':''} <span aria-hidden="true">${A.filtersOpen?'⌃':'⌄'}</span></button>${b('Reset','重置','reset')}<button type="submit" class="btn primary">${L('Search','查询')}</button></div></div>
      <div id="pr-advanced" class="pr-filters" ${A.filtersOpen?'':'hidden'}>${field(labels.owner,select('pr-f-owner',f.owner,[['',L('All','全部')],['A',L('Demo Asset A','示例资产企业 A')],['B',L('Demo Asset B','示例资产企业 B')]]))}${field(labels.project,select('pr-f-project',f.project,[['',L('All','全部')],['01',L('Demo project 01','示例融资项目 01')],['02',L('Demo project 02','示例融资项目 02')]]))}${field(labels.type,select('pr-f-type',f.type,[['',L('All','全部')],['first',L('Initial pledge','建池首笔')],['add',L('Additional pledge','追加')]]))}${field(L('Submitted from','提交开始日期'),input('pr-f-from',f.from,'date'))}${field(L('Submitted through','提交结束日期'),input('pr-f-to',f.to,'date'))}${field(labels.follow,select('pr-f-follow',f.follow,[['',L('All','全部')],...Object.entries(follows).filter(([k])=>k!=='none').map(([k,v])=>[k,txt(v)])]))}</div>
  ...
  function queue(){
    const stats=filtered(true),start=new Date(now());start.setHours(0,0,0,0);start.setDate(start.getDate()-6);
    const counts=[stats.filter(r=>r.status==='pending').length,stats.filter(r=>r.status==='timeout').length,stats.filter(r=>['approved','rejected'].includes(r.status)&&r.decisionAt>=start.getTime()).length];
    const state=A.view==='default'?null:surface();
    const metrics=`<div class="pr-metrics">${[['Pending review','待审核'],['Review timeout','质押失败（审核超时）'],['Reviewed · last 7 calendar days','近 7 个自然日人工审结']].map((x,i)=>`<div class="card pr-metric"><span>${txt(x)}</span><strong>${['loading','error'].includes(A.view)?'—':A.view==='empty'?0:counts[i]}</strong><span class="pr-meta">${L('applications','笔')}</span></div>`).join('')}</div><p class="pr-meta">${L('Summary uses company, project, type, submission dates and keyword filters. Status filters do not change this summary.','统计应用资产方、项目、类型、提交时间及关键词条件，不受审核状态和后续情况筛选影响。')}</p>`;
    let found=filtered();found.sort((a,c)=>{let av=A.sort==='waiting'?(a.decisionAt||a.ended||now())-a.submitted:a[A.sort],cv=A.sort==='waiting'?(c.decisionAt||c.ended||now())-c.submitted:c[A.sort];return av==null?1:cv==null?-1:(av-cv)*A.dir||a.id.localeCompare(c.id);});
    const pages=Math.max(1,Math.ceil(found.length/CF.PAGE_SIZE));if(A.page>pages){A.page=pages;A.error=['The previous page is empty; showing the last available page.','原页已无记录，已返回最后有效页。'];}
    let table=state||(!found.length?CF.empty(L('No applications found','暂无符合条件的申请'),L('Change the filters or return to pending reviews.','调整筛选条件，或返回待审核队列。'),b('Reset filters','重置筛选','reset')):`<div class="tablewrap"><table class="tbl pr-table pr-queue-table"><thead><tr><th>${L('Application / token','单笔 / 代币')}</th><th>${L('Asset holder','资产方')}</th><th>${L('Project','项目')}</th><th aria-sort="${A.sort==='value'?(A.dir===1?'ascending':'descending'):'none'}">${sortButton('value','Value (USD)','价值（USD）')}</th><th aria-sort="${A.sort==='deadline'?(A.dir===1?'ascending':'descending'):'none'}">${sortButton('deadline','Review deadline','审核截止')}</th><th>${L('Review status','审核状态')}</th><th class="col-act">${L('Action','操作')}</th></tr></thead><tbody>${found.slice((A.page-1)*CF.PAGE_SIZE,A.page*CF.PAGE_SIZE).map(r=>`<tr><td><strong class="mono">${r.id}</strong><span class="pr-meta mono">${r.token}</span></td><td>${E(owner(r))}</td><td>${E(project(r))}</td><td class="pr-num mono">${r.value==null?L('Unknown','待核实'):CF.fmtAmt(r.value)}</td><td>${time(r.deadline)}</td><td>${badge(r)}</td><td class="col-act">${link('View','查看','view',r.id)}</td></tr>`).join('')}</tbody></table></div><div class="pager"><span class="total">${L('Total','共')} ${found.length} ${L('applications','笔')} · ${CF.PAGE_SIZE} ${L('per page','笔／页')}</span>${b('Previous','上一页','page',A.page-1,'sm',A.page===1)}<span>${A.page} / ${pages}</span>${b('Next','下一页','page',A.page+1,'sm',A.page===pages)}</div>`);
    return title(L('Pledge reviews','代币质押审核'),L('Review each token application independently.','逐笔查看并处理代币质押申请。'))+metrics+`<section class="card">${filters()}<div class="pr-list-tools">${field(L('Sort by','排序'),select('pr-order',A.sort+':'+A.dir,[['submitted:1',L('Submitted · oldest first','提交时间 · 最早优先')],['submitted:-1',L('Submitted · newest first','提交时间 · 最新优先')],['waiting:-1',L('Waiting · longest first','审核等待 · 最长优先')],['waiting:1',L('Waiting · shortest first','审核等待 · 最短优先')],['value:-1',L('Value · highest first','价值 · 从高到低')],['value:1',L('Value · lowest first','价值 · 从低到高')],['deadline:1',L('Deadline · earliest first','审核截止 · 最早优先')],['deadline:-1',L('Deadline · latest first','审核截止 · 最晚优先')]]))}</div><div class="pr-section" ${A.error?'':'hidden'}>${error()}</div>${table}</section>`;
  }
  function title(t,sub){return `<div class="pr-title"><div><h1>${t}</h1><span class="pr-meta">${sub}</span></div>${CF.tag('gray',L('Demonstration data','演示数据'))}</div>`;}
  ...
  function uploadedFile(f,key,description,remove){return CF.fileRow({key,name:f.name,state:f.state,status:description,previewable:supportsPreview(f),disabled:A.busy,
    preview:{act:'pr-preview',value:key},download:{act:'pr-download',value:key},error:f.error?txt(f.error):'',retry:{act:'pr-file-retry',value:key},remove:remove?{act:'pr-remove',value:key.split(':')[1]}:null});}
  function fileList(r){return r.files.length?r.files.map((f,i)=>uploadedFile(f,'record:'+i,['invalid','expired'].includes(r.follow)?L('Invalidated · archive only','已失效 · 仅存档'):L('Linked to this decision','已关联本笔结论'),false)).join(''):note(L('No formal agreement for this application.','本笔暂无正式协议。'));}
  function activity(r){
    const heading=`<h2 class="pr-h2" id="pr-activity-title">${L('Activity','操作记录')}</h2>`;
    let body;
  ...
  function detail(){const r=row();if(!r)return CF.empty(L('Application unavailable','申请不可用'),L('Return to the review queue.','请返回审核队列。'),b('Back to queue','返回队列','back'));
    if(A.view!=='default')return title(E(r.id),'')+surface();
    const batch=A.rows.filter(x=>x.batch===r.batch),pending=batch.filter(x=>x.status==='pending').length;
    const related=A.rows.filter(x=>x.owner===r.owner&&x.project===r.project&&x.batch!==r.batch).sort((a,b)=>b.submitted-a.submitted);
    const summary=Object.entries(status).map(([k,v])=>`${txt(v)} ${batch.filter(x=>x.status===k).length}`).join(' · ');
    const permission=A.role==='read'?L('View access only.','当前仅有查询权限。'):r.status!=='pending'?L('This review has ended. The record is read-only.','本笔审核已结束，记录只读。'):L('Record your offline decision for this token.','请维护本笔线下审核结论。');
    const decision=['approved','rejected'].includes(r.status)?section('Decision and evidence','审核结论与凭据',kv([[L('Reviewer','审核人'),txt(r.operator)],[L('Decision time','结论时间'),time(r.decisionAt)],[L('Decision record','结论记录编号'),r.reviewId],[L('Decision value','结论时价值'),money(r.value)],...(r.status==='rejected'?[[L('Category','原因归类'),r.category!==''?txt(categories[+r.category]):'—'],[L('Rejection reason','驳回原因'),`<div class="pr-confirm">${E(r.reason)}</div>`,true]]:[]),...(r.remark?[[L('Internal note','内部备注'),E(r.remark),true]]:[])])+(r.status==='approved'?`<div class="pr-evidence" id="pr-evidence" tabindex="-1"><h3>${L('Agreement files','质押协议')}</h3>${fileList(r)}</div>`:'')):'';
    const overview=section('Application overview','申请概览',kv([[L('Asset holder','资产方'),E(owner(r))+'<span class="pr-meta mono">DEMO-ENTITY-'+r.owner+'</span>'],[L('Project','项目'),E(project(r))+'<span class="pr-meta mono">DEMO-PJ-'+r.project+'</span>'],[L('Project status / type','项目状态 / 类型'),L('Open · financing project','进行中 · 融资项目')],[L('Application type','申请类型'),r.type==='first'?L('Initial pledge','建池首笔'):L('Additional pledge','追加')],[L('Submitted','提交时间'),time(r.submitted)],[L('Batch identifier','批次编号'),r.batch]]))+
      section('Token details','代币详情',kv([[L('Token identifier','代币编号'),r.token],[L('Quantity / currency','数量 / 币种'),CF.fmtAmt(r.amount)+' '+r.currency],[L('Underlying tenor','底层账期'),r.term+' '+L('days','天')],[L('Buyer','买方企业'),r.buyer||L('To be verified','待核实')],[L('Validity','有效性'),r.valid?L('Valid','有效'):L('Invalid','已失效')]]))+decision+activity(r);
    const associations=section('Batch context','批次上下文',`<div class="pr-row"><b class="mono">${r.batch}</b>${CF.tag('gray',pending===batch.length?L('Pending review','待审核'):pending?L('Review in progress','审核进行中'):L('Review ended','审核已结束'))}</div><p class="pr-meta">${summary}</p><p class="pr-meta">${L('Approved applications — execution: ','已通过笔执行进度：')}${[['none',L('Not initiated','未发起')],['processing',L('Processing','处理中')],['success',L('Success','成功')],['failed',L('Failure','失败')]].map(([k,label])=>label+' '+batch.filter(x=>x.status==='approved'&&(k==='none'?['none','ready','invalid','expired'].includes(x.follow):k==='processing'?['processing','verify','difference'].includes(x.follow):x.follow===k)).length).join(' · ')}</p><div class="tablewrap"><table class="tbl pr-table"><thead><tr><th>${L('Token / application','代币 / 单笔')}</th><th>${L('Review','审核')}</th><th>${L('Reviewer / time','审核人 / 时间')}</th><th class="col-act">${L('Action','操作')}</th></tr></thead><tbody>${batch.map(x=>`<tr><td>${x.token}<span class="pr-meta mono">${x.id}</span></td><td>${badge(x)}</td><td>${x.operator?txt(x.operator):'—'}<span class="pr-meta">${time(x.decisionAt||x.ended)}</span></td><td class="col-act">${x.id===r.id?L('Current','当前'):link('View','查看','view',x.id)}</td></tr>`).join('')}</tbody></table></div>`)+
  ...
  function surface(){S.st=A.view;return CF.surface({emptyTitle:L('No applications yet','暂无申请'),emptyDesc:L('New applications will appear here.','收到新申请后将在此显示。'),deniedDesc:L('Contact the platform administrator to check access.','请联系平台建设方核对权限。'),backTo:ROOT,backLabel:L('Back to queue','返回队列'),skelRows:5});}
  function start(mode){expire();if(!canWrite(row()))return deny();A.draft={id:row().id,mode,files:[],reason:'',category:'',remark:'',ack:false};A.error='';open('edit');}
  function validate(){const d=A.draft;if(!d)return false;A.error='';if(d.mode==='approve'){
      ...
      else if(!d.ack)A.error=['Confirm that approval is not pledge success.','请确认知悉：审核通过不等于质押成功。'];
    }else{const s=d.reason.trim(),n=Array.from(s).length;if(n<10||n>500||!/[\p{L}\p{N}]/u.test(s))A.error=['Enter a reason of 10–500 characters; punctuation alone is not valid.','原因需为 10～500 字，不能仅填标点。'];}
    return !A.error;
  }
  function finalReason(d){return d.reason.trim();}
  function applyDecision(r,d,other=false){if(r.status!=='pending')return false;r.status=d.mode==='approve'?'approved':'rejected';r.decisionAt=now();r.operator=other?['Demo operator Chen','示例审核员陈']:['Demo operator Lin','示例审核员林'];r.reviewId=r.id.replace('PR','RV');r.reason=d.mode==='reject'?finalReason(d):'';r.category=d.category;r.remark=d.remark;r.files=d.mode==='approve'?d.files.map(f=>({...f})):[];r.entryDeadline=d.mode==='approve'?now()+7*86400000:null;r.follow=d.mode==='approve'?'ready':'none';event(r,status[r.status],d.mode==='approve'?['At approval: deposit pre-check and execution had not started; no execution fee.','通过时：尚未进行入池复核或发起执行，未产生执行费用。']:r.reason,r.operator,{id:r.reviewId,result:d.mode==='approve'?['Pending review → Approved · awaiting asset holder confirmation','待审核 → 已通过 · 待资产方确认入池']:['Pending review → Rejected','待审核 → 已驳回'],files:r.files.map(f=>({name:f.name}))});A.events.push({id:r.id,event:r.status});return true;}
  function submit(){expire();const d=A.draft,r=row();if(A.busy)return;if(!d||d.id!==r?.id||!canWrite(r))return deny();if(!validate()){open('edit');return;}A.busy=true;A.error='';CF.render();const epoch=A.epoch,response=A.response;
    setTimeout(()=>{if(epoch!==A.epoch||A.draft!==d)return;expire();A.busy=false;if(!canWrite(r))return deny();
      ...
      if(response==='withdraw'){r.status='withdrawn';r.ended=now();event(r,['Withdrawn by asset holder','资产方已撤回'],'',['Asset holder','资产方']);A.draft=null;S.layer=null;A.error=['The asset holder withdrew this application. No decision was saved.','资产方已撤回本笔，未保存审核结论。'];CF.render();return;}
  ...
  function editLayer(){const d=A.draft,r=row();if(!d||!r)return {title:L('Unavailable','不可用'),html:'',foot:b('Close','关闭','cancel')};
    ...
    const body=d.mode==='approve'?`${field(L('Agreement files','质押协议'),`<label class="btn pr-upload" for="pr-upload">${L('Upload agreements','上传协议')}<input class="sr-only" id="pr-upload" type="file" accept="application/pdf,image/jpeg,image/png" multiple></label>`)}<span class="pr-meta">${L('PDF / JPG / PNG · 1–5 files · up to 10 MB each','PDF / JPG / PNG · 1～5 个 · 每个不超过 10 MB')}</span>${docs}${field(L('Internal note (optional)','内部备注（选填）'),`<textarea class="inp" id="pr-remark" aria-invalid="${!!A.error&&Array.from(d.remark).length>200}">${E(d.remark)}</textarea><span class="pr-meta">${Array.from(d.remark).length} / 200</span>`)}<label><input id="pr-ack" type="checkbox" ${d.ack?'checked':''}> ${L('I understand that approval does not mean the pledge has succeeded.','我已知悉：审核通过不等于质押成功。')}</label>`:
      `${field(L('Category (optional)','归类（选填）'),select('pr-category',d.category,[['',L('Not selected','未选择')],...categories.map((x,i)=>[String(i),txt(x)])]))}${field(L('Reason shown to asset holder','向资产方展示的原因'),`<textarea class="inp" id="pr-reason" aria-invalid="${!!A.error}" aria-describedby="pr-form-error">${E(d.reason)}</textarea><span class="pr-meta">${Array.from(d.reason.trim()).length} / 500 · ${L('Minimum 10 characters','至少 10 字')}</span>`)}`;
    return {title:d.mode==='approve'?L('Approve application','通过本笔申请'):L('Reject application','驳回本笔申请'),html:`<div class="pr-form">${identity(r)}<fieldset ${A.busy?'disabled':''}>${body}</fieldset><div id="pr-form-error">${error()}</div>${A.busy?'<p role="status">'+L('Saving decision…','正在保存结论…')+'</p>':''}</div>`,foot:b('Cancel','取消','cancel','','',A.busy)+(d.mode==='approve'?b('Approve this application','通过本笔申请','submit','','primary',A.busy):b('Review decision','核对结论','confirm','','primary',A.busy))};
  }
  const layers={
    'pr-edit':editLayer,
    'pr-confirm':()=>({title:L('Confirm rejection','确认驳回'),html:identity(row())+`<div class="pr-section"><b>${L('Reject this application only','仅驳回本笔申请')}</b><div class="pr-confirm">${E(A.draft?.reason.trim())}</div><p class="pr-meta">${L('The reason above will be shown in full to the asset holder. Other applications are unchanged.','以上原因将全文展示给资产方，其他申请不受影响。')}</p>${A.busy?'<p role="status">'+L('Saving decision…','正在保存结论…')+'</p>':''}</div>`,foot:b('Back','返回修改','edit','','',A.busy)+b('Confirm rejection','确认驳回','submit','','primary',A.busy)}),
    'pr-discard':()=>({title:L('Discard unsaved inputs?','放弃未提交内容？'),html:note(L('Files, notes and reasons will not be carried to another application.','未提交文件、备注及原因不会带入其他申请。')),foot:b('Keep editing','继续编辑','keep')+b('Discard','放弃内容','discard','','primary')}),
    'pr-preview':()=>({title:L('Agreement preview','协议预览'),html:previewHTML(),foot:b('Close','关闭','close')+b('Download','下载','download',A.previewKey,'primary')})
  };
  function sampleURL(){const stream='BT /F1 18 Tf 40 150 Td (DEMONSTRATION AGREEMENT) Tj 0 -30 Td (Covers the DEMO tokens in the linked applications.) Tj ET';const objs=['<< /Type /Catalog /Pages 2 0 R >>','<< /Type /Pages /Kids [3 0 R] /Count 1 >>','<< /Type /Page /Parent 2 0 R /MediaBox [0 0 500 220] /Resources << /Font << /F1 4 0 R >> >> /Contents 5 0 R >>','<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>',`<< /Length ${stream.length} >>\nstream\n${stream}\nendstream`];let pdf='%PDF-1.4\n',offsets=[0];objs.forEach((o,i)=>{offsets.push(pdf.length);pdf+=`${i+1} 0 obj\n${o}\nendobj\n`;});const x=pdf.length;pdf+='xref\n0 6\n0000000000 65535 f \n'+offsets.slice(1).map(n=>String(n).padStart(10,'0')+' 00000 n \n').join('')+`trailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n${x}\n%%EOF`;return URL.createObjectURL(new Blob([pdf],{type:'application/pdf'}));}
  let sample;
  function fileURL(f){if(f.sample)return sample||(sample=sampleURL());return f.url;}
  function previewHTML(){if(!canRead())return note(L('Access denied.','无权查看。'));const f=getFile(A.previewKey);if(!f)return note(L('File unavailable.','文件不可用。'));if(A.preview==='missing')return note(L('File could not be retrieved. Retry later.','文件暂时不可得，请稍后重试。'))+b('Retry','重试','preview-retry');if(A.preview==='fail')return note(L('Preview failed. Download the file to view it.','预览失败，请下载后查看。'))+b('Retry preview','重试预览','preview-retry');if(f.error)return note(E(txt(f.error)))+b('Retry download','重试下载','file-retry',A.previewKey);if(f.sample)return `<div class="pr-confirm"><h3>${L('Demonstration agreement','演示协议')}</h3><p>${E(row().batch)} · ${E(row().token)}</p><p>${L('This demonstration document covers all DEMO tokens in this batch.','此演示材料覆盖本批次全部 DEMO 代币。')}</p></div>`;return /image/.test(f.type)?`<img class="pr-preview" src="${E(fileURL(f))}" alt="${E(f.name)}">`:`<object class="pr-preview" type="application/pdf" data="${E(fileURL(f))}">${L('Download the PDF to view it.','请下载 PDF 查看。')}</object>`;}
  ...
  function onAct(act,v,e){if(e&&(e.ctrlKey||e.metaKey||e.shiftKey||e.altKey)&&['pr-view','pr-origin'].includes(act))return false;if(!act.startsWith('pr-')){if(act==='retry'){A.view='default';A.error='';CF.render();return true;}if(act==='clearfilter'){resetFilter();return true;}return false;}act=act.slice(3);if(A.busy)return true;
    ...
    else if(act==='confirm'){expire();if(!canWrite(row()))deny();else if(validate()){if(A.draft.mode==='approve')submit();else open('confirm');}else{CF.render();document.getElementById(A.draft.mode==='reject'?'pr-reason':'pr-upload')?.focus();}}
    ...
    else if(act==='close')closePreview();else if(act==='preview'){if(!canRead())deny();else{const f=getFile(v);if(!f||f.state!=='ready'||!supportsPreview(f))return true;A.previewKey=v;A.previewReturn={draft:!!A.draft,key:v,y:window.scrollY,scroll:document.querySelector('.modal-b')?.scrollTop||0};open('preview');}}else if(act==='download')download(v);
    else if(act==='file-retry'){const f=getFile(v);if(f){f.error=null;A.preview='success';download(v);CF.render();}}
    else if(act==='evidence'){document.getElementById('pr-evidence')?.scrollIntoView({block:'center'});document.getElementById('pr-evidence')?.focus({preventScroll:true});}
    else if(act==='preview-retry'){A.preview='success';const f=getFile(A.previewKey);if(f)f.error=null;CF.render();}
    else if(act==='activity-retry'){A.activityState='default';CF.render();document.getElementById('pr-activity')?.focus({preventScroll:true});}
    ...
    else if(act==='forbidden'){if(A.role==='review')CF.toast(L('Choose a restricted permission profile first.','请先选择受限权限。'));else deny();}
    else if(act==='resolve'){const u=A.uncertain,r=A.rows.find(x=>x.id===u?.id);if(!canRead())deny();else{if(r?.status==='pending'&&now()<r.deadline)applyDecision(r,u.d);A.uncertain=null;A.error='';CF.render();CF.toast(L('Current result retrieved.','已获取当前结果。'));}}
    else if(act==='late-result'){const r=row();if(r?.follow==='verify'){r.follow='success';r.completed=now();r.fee=0.0012;r.tx='DEMO-LATE-TX-028';event(r,['Execution succeeded','执行成功'],['Delayed result received for this execution.','本次执行已收到迟到结果。'],['System','系统'],{id:r.id+':late-success',result:['Under verification → Succeeded','待核实 → 执行成功'],execution:true,started:r.started,completed:r.completed,fee:r.fee,feeCurrency:r.feeCurrency,tx:r.tx});CF.render();}}
    else if(act==='replay-event'){const r=row();if(r&&r.events.length){const latest=r.events.slice().sort((a,b)=>(b.at||0)-(a.at||0))[0];r.events.push({...latest});CF.render();}}
    else if(act==='restart'){A.epoch++;seed=CF.pledgeSeed();A.rows=seed.rows;A.now=seed.now;A.clock=Date.now();A.draft=null;A.uncertain=null;A.busy=false;A.role='review';A.response='success';A.upload='success';A.preview='success';A.activityState='default';A.events=[];resetFilter();navigate(null);}
    return true;
  }
  ...
  document.addEventListener('change',e=>{onInput(e);const el=e.target,id=el.id;if(id==='pr-order'){const [sort,dir]=el.value.split(':');A.sort=sort;A.dir=+dir;saveContext();CF.render();}
    if(['pr-role','pr-view','pr-response','pr-upload-state','pr-preview-state','pr-activity-state'].includes(id)){const key={'pr-role':'role','pr-view':'view','pr-response':'response','pr-upload-state':'upload','pr-preview-state':'preview','pr-activity-state':'activityState'}[id];A[key]=el.value;A.error='';if(key==='role'){A.epoch++;A.busy=false;S.layer=null;A.draft=null;}CF.render();}
    if(id==='pr-upload'&&A.draft&&!A.busy){const d=A.draft,files=Array.from(el.files);A.error='';if(files.length+d.files.length>5){A.error=['At most 5 files per application.','每笔最多 5 个文件。'];CF.render();return;}
  ...
  CF.define({id:'pledge-review',beforeAdminNavigate(proceed){if(A.busy||A.draft){CF.toast(L('Finish or cancel the current review first.','请先完成或取消当前审核。'));return}remember();proceed()},dict,content,layers,onAct,demoOnly:true,demo,
    ...
    onBeforeAct(act,v,e){if(act==='retry'||act==='clearfilter'){onAct(act,v);return true;}if(act==='closelayer'&&e?.type==='click'&&e.target.closest('[data-stop]'))return false;if(act==='closelayer'&&A.busy)return true;if(act==='closelayer'&&S.layer?.key==='pr-preview'){closePreview();return true;}if(act==='closelayer'&&A.draft){onAct('pr-cancel','');return true;}return false;},
