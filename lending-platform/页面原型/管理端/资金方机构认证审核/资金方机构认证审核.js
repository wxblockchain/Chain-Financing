...
(function(CF){
  ...
  function detail(){
    ...
    return `<div class="review-wrap">${title('Application details','机构认证申请详情',E(A.selected))}
      ...
      <section class="card rv-review-records" aria-labelledby="rv-history-title"><div class="card-b"><h2 id="rv-history-title">${L('Review records','审核记录')}</h2></div><div class="tablewrap"><table class="tbl rv-record-table"><thead><tr>${[['Submitted by','提交人'],['Submitted','提交时间'],['Reviewed by','审核人'],['Reviewed','审核时间'],['Result','审核结果'],['Rejection reason','驳回原因']].map(h=>`<th>${L(...h)}</th>`).join('')}</tr></thead><tbody>${a.records.slice().reverse().map(h=>`<tr><td>${E(h.submitter)}</td><td class="rv-meta">${time(h.submitted)}</td><td>${E(h.reviewer||'—')}</td><td class="rv-meta">${time(h.reviewed)}</td><td>${tag(h.status)}</td><td>${h.status==='rejected'?problemList(h):'—'}</td></tr>`).join('')}</tbody></table></div></section></div>`;
