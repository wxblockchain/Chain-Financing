...
(function(CF){
  ...
  function list(){
    ...
    if(S.st==='loading')body=`<div role="status">${CF.skelTable(5)}</div>`;
      ...
      body=`<div class="tablewrap"><table class="tbl rv-table"><thead><tr>${[['Application','申请编号'],['Institution','机构标识 / 名称'],['Version','版本'],['Submitted ↑','提交时间 ↑'],['Status','状态'],['Decision time','结论时间'],['Action','操作']].map((h,i)=>`<th ${i===6?'class="col-act"':''} ${i===3?'aria-sort="ascending"':''}>${L(...h)}</th>`).join('')}</tr></thead><tbody>${found.slice((A.page-1)*5,A.page*5).map(a=>{const f=a.submittedForm||a.form;return `<tr><td class="mono">${a.id}</td><td><b>${E(f.name)}</b><div class="rv-meta mono">${E(f.identifier)}</div></td><td class="num">V${a.version}</td><td class="rv-meta">${time(a.submitted)}</td><td>${tag(a.status)}</td><td class="rv-meta">${time(a.reviewed)}</td><td class="col-act">${link('View','查看','view',a.id)}</td></tr>`;}).join('')}</tbody></table></div><div class="rv-foot"><span class="rv-meta">${L(`${found.length} applications`,`${found.length} 条申请`)}</span><div>${btn('Previous','上一页','page',String(A.page-1),'',A.page===1)} <span class="rv-meta">${A.page} / ${pages}</span> ${btn('Next','下一页','page',String(A.page+1),'',A.page===pages)}</div></div>`;
    }
  ...
  function detail(){
    ...
    const old=v.version!==a.version,fields=v.fields||a.submittedFields||[];
    return `<div class="review-wrap">${link('← Back to applications','← 返回申请列表','back')}${title('Application details','机构认证申请详情',A.selected+' · V'+v.version)}
      ...
      ${canReview()?`<div class="rv-actions"><span>${L('Apply the decision to this submitted version.','结论仅作用于本次提交版本。')}</span><div>${btn('Reject','驳回','reject')} ${btn('Approve','通过','approve','','primary')}</div></div>`:CF.note('accent',L(old?'Historical versions cannot be reviewed.':v.status==='submitted'?'You have view-only access.':'This version has a final decision and is read only.',old?'历史版本不可再次处置。':v.status==='submitted'?'当前权限仅可查询。':'该版本已出具结论，仅可查看。'))}</div></section></div>
      <aside class="card"><div class="card-b"><h2>${L('Version history','版本历史')}</h2>${[{...a},...a.history.slice().reverse()].map(h=>`<div class="rv-history"><div>${link('Version '+h.version,'版本 '+h.version,'version',String(h.version))} ${tag(h.status)}</div><p class="rv-meta">${time(h.submitted)}</p><p class="rv-meta">${h.version===a.version?L('Current submission','当前提交'):L('Historical submission','历史提交')}</p></div>`).join('')}</div></aside></div></div>`;
  }
  ...
  function action(act,v){
    ...
    switch(act.slice(3)){
      ...
      case 'version':A.version=+v;A.error='';break;
