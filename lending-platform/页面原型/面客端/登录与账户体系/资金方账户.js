...
(function(CF){
  ...
  const blank=()=>({exists:false,address,created:null,email:'',status:'draft',disabled:false,form:{name:'',country:'',identifierType:'',identifier:'',institutionType:'',registeredAddress:'',regulator:'',license:'',contact:'',file:'',fileSize:0,fileStatus:''},submitted:null,reviewed:null,records:[],version:0});
  const F={account:blank(),accounts:{},connected:'',change:false,inFlow:false,
    ...
    uploadVersion:0,preferenceState:'',savedPreferences:null,storageFailed:false,downgrade:false,latestCode:'123456',sends:{},pendingSubmission:false,emailFlow:false,notes:[],generation:0,layerOpen:false,focusBack:null};
  ...
  function hydrate(a){F.account=F.accounts[a]?JSON.parse(JSON.stringify(F.accounts[a])):blank();F.account.form={...blank().form,...(F.account.submittedForm||{})};F.account.address=a;normalizeRecords(F.account);F.notes=F.account.notes||[];F.pendingSubmission=!!F.account.pending;F.downgrade=!!F.account.reviewPending;F.change=false;F.emailFlow=false;F.otp=null;F.fieldErrors={};F.emailInput=F.account.email;F.upload=F.account.form.file?(F.account.form.fileStatus||'done'):'idle';if(F.upload==='loading'){F.upload='failed';F.account.form.fileStatus='failed';}}
  ...
  function materialRow(form,source='current',editable=false){
    if(!form?.file)return `<p class="login-caption">${L('No file uploaded','尚未上传文件')}</p>`;
    const state=editable?F.upload:(form.fileStatus||'done');
    return CF.fileRow({key:'institution-'+source,name:form.file,meta:L('Institution document','机构证明材料')+' · '+(form.fileSize?fileSize(form.fileSize):L('Size unavailable','大小未记录')),state:state==='done'?'ready':state==='loading'?'uploading':'failed',status:state==='done'?L('Uploaded','已上传'):state==='loading'?L('Uploading…','正在上传…'):L('Upload failed','上传失败'),previewable:materialPreviewable(form),preview:{act:'f-material-preview',value:source},download:{act:'f-material-download',value:source},disabled:!!F.busy||F.pendingSubmission,error:state==='failed'?L('Retry or replace the file.','请重试或替换文件。'):'',retry:editable?{act:'f-upload'}:null,replace:editable?{act:'f-pick-file',disabled:state==='loading'}:null,remove:editable?{act:'f-remove-file'}:null});
  }
  function materialPreviewable(form){return /\.(pdf|png|jpe?g|webp)$/i.test(form?.file||'');}
  function materialPreview(form,data){
    if(!form?.file||form.fileStatus!=='done')return CF.note('warn',L('File unavailable.','文件不可用。'));
    if(!form.fileDemo)return CF.note('warn',L('The original file is unavailable in this prototype. Please retry later.','当前原型无法读取此材料原件，请稍后重试。'));
    return `<div class="funder-material-preview"><h3>${L('Demonstration document','演示材料')}</h3><p>${esc(form.file)}</p>${details((data||[]).filter(f=>f[0]!=='Institution document').slice(0,5),true)}</div>`;
  }
  function downloadMaterial(form){
    if(!form?.fileDemo||form.fileStatus!=='done'){CF.toast(L('Download unavailable: the original file could not be retrieved. Please retry later.','下载失败：当前无法获取材料原件，请稍后重试。'));return false;}
    ...
    const url=URL.createObjectURL(new Blob([pdf],{type:'application/pdf'})),link=document.createElement('a');link.href=url;link.download=form.file;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);return true;
  }
  function uploadArea(){return `<div class="field"><span class="funder-field-label">${L('Institution document *','机构证明材料 *')}</span><div class="funder-upload" id="f-material" tabindex="-1" ${F.fieldErrors.file?'aria-describedby="f-err-file"':''}><input id="f-file" type="file" hidden aria-label="${L('Select institution document','选择机构证明材料')}">${F.account.form.file?materialRow(F.account.form,'draft',true):`<div class="funder-upload-empty"><span class="funder-file-icon" aria-hidden="true">↑</span><div><strong>${L('Upload institution document','上传机构证明材料')}</strong><p class="login-caption">${L('Choose a file from your computer.','从电脑选择需要提交的文件。')}</p></div>${btn('Choose file','选择文件','pick-file')}</div>`}${F.fieldErrors.file?`<p id="f-err-file" class="err-msg" role="alert">${L(...F.fieldErrors.file)}</p>`:''}</div></div>`;}
  function startUpload(file){if(F.account.status==='submitted'||F.pendingSubmission)return;delete F.fieldErrors.file;const version=++F.uploadVersion;if(file){F.account.form.fileDemo=false;F.account.form.file=file.name;F.account.form.fileSize=file.size;}F.upload='loading';F.account.form.fileStatus='loading';later(()=>{if(version!==F.uploadVersion)return;F.upload=F.saveResult==='failed'?'failed':'done';F.account.form.fileStatus=F.upload;});}
  ...
  function readiness(){
    ...
    const file=!a.form.file?['Not uploaded','未上传','warn']:F.upload==='done'?['Uploaded','已上传','ok']:F.upload==='loading'?['Uploading…','正在上传…','warn']:['Upload failed','上传失败','danger'];
    return `<dl class="login-fields funder-detail-grid funder-readiness">
      ...
      <div><dt>${L('Institution document','机构证明材料')}</dt><dd>${CF.tag(file[2],L(file[0],file[1]))}</dd></div></dl>`;
  }
  ...
  function statusPage(){
    ...
    const feedback=a.status==='rejected'
      ...
      : a.status==='submitted'?CF.note('accent',`<b>${L('Under review','审核中')}</b><p>${L('Institution details are locked until a review decision is available.','机构资料在审核结论出具前不可修改。')}</p>${link('Refresh status','刷新状态','refresh')}`):'';
    return `<div class="funder-wrap"><div class="page-head"><div><h1 class="page-title">${L('Registration status','注册状态')}</h1><p class="page-desc">${L('View your institution details and verification status.','查看机构资料及认证状态。')}</p></div>${tag(a.status)}</div>
      ...
      <div class="funder-status-stack">${feedback}
        ...
        <section class="card" id="f-institution-summary"><div class="card-b"><h2>${L('Institution details','机构资料')}</h2>${details((view.fields||[]).filter(f=>f[0]!=='Institution document'),true)}${materialRow(a.submittedForm,'current')}${a.status==='verified'?`<div class="login-actions">${btn('Update institution details','变更机构资料','change','','primary')}</div>`:''}</div></section>
        ${stamp()}
      </div></div>`;
  }
  ...
  function discardDraft(){
    ...
    a.form={...blank().form,...(a.submittedForm||{})};
    F.upload=a.form.file?(a.form.fileStatus||'done'):'idle';
  ...
  function fixture(s){
    ...
    if(s!=='draft'){a.email=a.email||'finance@example.test';a.emailVerified=a.emailVerified||new Date().toISOString();a.form={name:'Demo Institution',country:'Demo jurisdiction',identifierType:'REG_NO',identifier:'DEMO-REG-100',institutionType:'Demo financial institution',registeredAddress:'Demo registered address',regulator:'',license:'',fileSize:204800,fileStatus:'done',contact:'contact@example.test',fileDemo:true,file:'institution-demo.pdf'};F.upload='done';a.submitted=a.submitted||new Date().toISOString();a.version=Math.max(1,a.version);a.snapshotEmail=a.email;a.submittedFields=fields();a.submittedForm=JSON.parse(JSON.stringify(a.form));a.template=F.activeTemplate||'DEMO-1';}
  ...
  function action(act,v){
    ...
    switch(key){
      ...
      case 'issue-field':go('/funder/register');setTimeout(()=>{const target=v==='file'?$('f-material'):v==='template'?$('f-name'):$('f-'+v);target?.scrollIntoView({block:'center'});target?.focus();},50);break;
      case 'gate':gate();break;
      case 'gate-next':F.returnTo='valid';go(F.account.status==='draft'?'/funder/register':'/funder/status');break;
      case 'material-preview':if(materialPreviewable(materialSource(v)))open('material',v);break;
      case 'material-download':downloadMaterial(materialSource(v));break;
      case 'pick-file':$('f-file')?.click();break;
      case 'upload':startUpload();break;
      case 'remove-file':if(F.account.status==='submitted'){CF.toast(L('Institution details are locked during review.','审核中机构资料不可修改。'));break;}F.uploadVersion++;F.account.form.file='';F.account.form.fileSize=0;F.account.form.fileStatus='';F.upload='idle';CF.toast(L('File deleted','文件已删除'));break;
  ...
  document.addEventListener('click',e=>{
    ...
    if(a.dataset.act==='f-pick-file'){e.preventDefault();e.stopImmediatePropagation();if(signedIn()&&!F.account.disabled&&F.account.status!=='submitted'&&!F.pendingSubmission&&!F.busy)$('f-file')?.click();return;}
  ...
  layers['f-material']=source=>({title:L('Preview','预览'),html:signedIn()&&!F.account.disabled?materialPreview(materialSource(source),source==='draft'?fields():source==='pending'?F.account.pending?.fields:F.account.submittedFields):CF.note('warn',L('Sign in to view this file.','请登录后查看文件。')),foot:btn('Close','关闭','close')+btn('Download','下载','material-download',source)});
