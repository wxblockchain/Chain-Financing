...
(function(CF){
  ...
  const blank=()=>({exists:false,address,created:null,email:'',status:'draft',disabled:false,step:1,form:{name:'',country:'',identifierType:'',identifier:'',institutionType:'',registeredAddress:'',regulator:'',license:'',contact:'',file:'',fileSize:0,fileStatus:''},submitted:null,reviewed:null,records:[],version:0});
  ...
  function startUpload(file){if(F.account.status==='submitted'||F.pendingSubmission)return;delete F.fieldErrors.file;const version=++F.uploadVersion;if(file){F.account.form.fileDemo=false;F.account.form.file=file.name;F.account.form.fileSize=file.size;}F.upload='loading';F.account.form.fileStatus='loading';persist();later(()=>{if(version!==F.uploadVersion)return;F.upload=F.saveResult==='failed'?'failed':'done';F.account.form.fileStatus=F.upload;persist();});}
  ...
  Object.keys(pages).filter(id=>!id.startsWith('DEMO-')).forEach(id=>CF.review.register(id,{
    ...
    states:()=>S.role!=='fund'||id==='P-L20'?['default']:['default','loading','error',...(id==='P-L22'?[
    ...
    step:id==='P-L21'?()=>L('Registration · step ','机构注册 · 第 ')+F.account.step:undefined,
  ...
  function action(act,v){
    ...
    switch(key){
      ...
      case 'register':F.submitConsent=false;F.change=false;if(F.account.status==='rejected')F.account.step=F.account.email?2:1;go('/funder/register');break;
  ...
  document.addEventListener('click',e=>{
    ...
    if(a.dataset.act==='f-pick-file'){e.preventDefault();e.stopImmediatePropagation();if(S.role==='fund'&&!F.account.disabled&&Date.now()<F.sessionUntil&&F.account.status!=='submitted'&&!F.pendingSubmission&&!F.busy)$('f-file')?.click();return;}
  ...
  layers['f-material']=source=>({title:L('Preview','预览'),html:S.role==='fund'&&!F.account.disabled&&Date.now()<F.sessionUntil?materialPreview(materialSource(source),source==='draft'?fields():source==='pending'?F.account.pending?.fields:F.account.submittedFields):CF.note('warn',L('Sign in to view this file.','请登录后查看文件。')),foot:btn('Close','关闭','close')+btn('Download','下载','material-download',source)});
