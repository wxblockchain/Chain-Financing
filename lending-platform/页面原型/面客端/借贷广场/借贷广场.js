...
(function(CF){
  ...
  let selection=[],name='',kind='',rate='',term='',settle=[],editing=null,error='',amount='',busy=false,appId=null,operation='deposit',pages={tokens:1,demands:1,select:1},filter={},pendingAction=null;
  ...
  const stateNames={projectReview:['Under review','待审核'],returned:['Returned','已退回'],available:['Available','可用'],raising:['Fundraising','募集中'],locked:['Locked','已锁定'],financing:['Financing','融资中'],closed:['Closed','已关闭'],settled:['Settled','已结清'],review:['Under review','审核中'],approved:['Approved','审核通过'],rejected:['Rejected','已驳回'],withdrawn:['Withdrawn','已撤回'],void:['Ended with the project','随项目终结'],executing:['Execution in progress','执行处理中'],recorded:['Results recorded','结果已回写'],processing:['Processing','处理中'],success:['Succeeded','成功'],failure:['Failed','失败'],noFee:['Failed','失败'],open:['Awaiting quotes','待报价'],quoted:['Quoted · awaiting confirmation','已报价待确认'],funding:['Funding in progress','放款中'],funded:['Funded','已放款'],ended:['Expired / closed','已失效／已关闭'],surplus:['Coverage surplus','覆盖有余'],balanced:['Coverage balanced','覆盖持平'],short:['Coverage shortfall','覆盖不足']};
  ...
  /* 上链前一次讲清：本次发生链上操作、需本人签名、gas 自担且失败不退。 */
  function chainNotice(){return note(L('This sends an on-chain transfer to the project pledge contract. You sign it yourself; gas is paid by your company and is not refunded if the transfer fails.','本次向本项目质押合约发起链上转入，需你本人签名；gas 由本企业承担，上链失败已产生的 gas 同样不退。'),'warn');}
  ...
  function matching(){return D.projects.filter(p=>!['review','returned','available'].includes(p.state)||Q&&Q.hasHistory(p)).filter(p=>{
  ...
  function stats(p){const n=D.numbers(p),d=D.current(p);
    ...
    const cells=[
      [L('Total pledged value','总质押额'),money(n.total),L('everything inside the contract','合约内全部代币'),''],
      [L('Pending review value','待审质押额'),money(n.pendingValue),L('on chain, not yet counted','已上链未计入有效'),n.pendingValue?'warn':''],
      [L('Eligible pledge amount','有效质押额'),money(n.limit),L('eligible value × pledge rate','有效质押价值 × 质押率'),'accent'],
      ...
      [L('Project pledge rate','项目质押率'),Math.round(n.rate*1000)/10+'%',L('locked at creation','创建时锁定'),'']];
  ...
  /* 上链后资产方要处理的只有被驳回、需自助解押取回的代币。 */
  function actionableApplications(p){return D.mine(p)?D.applications.filter(a=>a.project===p.id&&a.state==='rejected'&&D.token(a.tokens[0])?.pool===p.id):[];}
  ...
  function pledgeBody(p){
    ...
    body+='<div class="ls-flow-grid">'
      ...
      +btn('ls-review',L('Pledge review','质押审核'))
  ...
  function unavailableReason(a){const t=D.token(a.tokens[0]),e=executionFor(a);if(t.frozen)return L('Discrepancy; new actions are paused.','存在差异，暂停新操作。');if(t.pending||e?.state==='processing')return L('Awaiting execution results.','等待执行结果，暂不可再次操作。');if(e?.state==='success')return L('This application has already succeeded.','本笔已成功，不可重复入池。');return a.state==='review'?L('Awaiting review.','尚未通过审核。'):a.state==='expired'?L('The deposit deadline has passed.','已超过本笔入池截止时间。'):a.state==='void'?txt(reasons[a.reason]||reasons.recheck):a.state==='recorded'?L('Execution ended; submit a new application if eligible.','本次执行已结束，可按资格重新申请。'):L('This token needs to be released back before it can be pledged again.','本张需先自助解押取回后才能重新质押。');}
  function reviewTiming(a){return small(L('On chain: ','上链：')+time(a.submitted))+(a.reviewState==='approved'?small(L('Approved: ','通过：')+time(a.approvedAt)):a.decided?small(L('Reviewed: ','审核：')+time(a.decided)):a.finished?small(L('Ended: ','结束：')+time(a.finished)):'');}
  ...
  function reviewListStatus(a){return reviewLabel(a);}
  ...
  /* 签名前逐张复核资格：任何一张变化就更新集合，由用户确认后再签名。 */
  function validateDeposit(){
    const p=project(),changes=[];
    selection.forEach(id=>{const t=D.token(id),old=feeQuote.find(f=>f.id===id);
      if(!D.selectable(t,p)){changes.push(id+' · '+L('No longer eligible for pledge.','已不可质押。'));return;}
      const fee=D.quoteFee(id);if(!old||old.amount!==fee.amount||old.currency!==fee.currency)changes.push(id+' · '+L('Estimated gas changed.','预估 gas 已更新。'));
    });
    if(changes.length){selection=selection.filter(id=>D.selectable(D.token(id),p));feeQuote=feeSnapshot();confirmationChanges=changes;error='';return false;}
  ...
  function detailPage(){const p=project();if(!p||['review','returned','available'].includes(p.state)&&!D.mine(p)&&!(Q&&Q.hasHistory(p)))return publicMissing();const n=D.numbers(p);let surface=CF.surface({emptyTitle:L('No project data','暂无项目数据'),emptyDesc:'',backTo:'/marketplace'});if(surface!==null)return surface;
    ...
    if(p.state==='review'&&D.mine(p))warning+=note(L('This project is under review. The pledge contract is deployed once it is approved; you cannot pledge or publish yet.','本项目正在审核，通过后由平台部署质押合约；此前不能质押，也不能发布融资需求。'));
    if(p.state==='returned'&&D.mine(p))warning+=note(L('Returned: ','审核退回：')+E(txt(p.reviewReason||['',''])) +' '+btn('ls-amend',L('Amend and resubmit','修改并重新提交'),p.id),'warn');
    if(p.state==='available'&&!n.value&&D.mine(p))warning+=note(L('This project has no valid collateral. Complete a token pledge before publishing a financing request.','本项目暂无有效质押，请先完成代币质押后再发布。'),'warn');
  ...
  const kindLabel=id=>txt(D.type(id).label);
  function newPage(){if(S.role!=='asset')return publicMissing();
    ...
    const cfg=D.type(kind||kinds[0]);
    const options=none?[['','No token type available','暂无可选代币类型']]
      :[['','Choose a type','请选择代币类型'],...kinds.map(k=>[k,txt(D.type(k).label),txt(D.type(k).label)])];
    const back=editing?D.project(editing):null;
    return '<div class="ls-form ls-stack"><h1 class="page-title">'+(back?L('Resubmit financing project','修改并重新提交'):L('Create financing project','创建融资项目'))+'</h1>'
      +(back&&back.reviewReason?note(L('Returned: ','审核退回：')+E(txt(back.reviewReason)),'warn'):'')
      +card(L('Project information','项目信息'),
        ...
        +field('ls-rate','Pledge rate (%) *','项目质押率（%）*',rate,'text','inputmode="decimal" required aria-describedby="ls-rate-help"')
        +'<p class="hint" id="ls-rate-help">'+L('Up to '+(cfg.maxRate*100)+'%. Locked once the project is created.','不超过 '+(cfg.maxRate*100)+'%，创建后终身锁定。')+'</p>'
        +field('ls-term','Project term (months) *','项目期限（月）*',term,'text','inputmode="numeric" required aria-describedby="ls-term-help"')
        +'<p class="hint" id="ls-term-help">'+L('3 to '+cfg.maxTerm+' months, counted from the creation time.','3～'+cfg.maxTerm+' 个月，自创建时间起算。')+'</p>'
        +'<div class="field"><p class="field-label">'+L('Reference settlement currency *','参考结算币种 *')+'</p><div class="ls-row">'
        +D.SETTLE.map(c=>'<label class="ls-check"><input type="checkbox" data-settle="'+c[0]+'"'+(settle.includes(c[0])?' checked':'')+'><span>'+E(txt(c))+'</span></label>').join('')
        +'</div><p class="hint">'+L('For reference only; it does not limit the currencies funders may quote.','仅供参考，不限制资金机构报价币种。')+'</p></div>'
        +'<div class="field"><p class="field-label" id="ls-repay-label">'+L('Repayment method','还款方式')+'</p><p class="field-static" role="note" aria-labelledby="ls-repay-label">'+E(txt(D.REPAY))+'</p></div>'
      ...
      +err()
      +'<div class="ls-form-foot"><p class="hint">'+L('Submitting starts the project review. No tokens are selected, nothing goes on chain and no fee applies.','提交后进入项目审核：不勾选代币、不发生链上操作、不产生费用。')+'</p>'
      +'<div class="ls-row">'+btn('ls-create',L('Submit for review','提交审核'),'',true,busy||none)+btn('ls-list',L('Cancel','取消'))+'</div></div></div>';}
  ...
  function openPublish(){const p=project();if(!guarded(p))return;const a=D.actions(p);if(!a.edit&&!a.publish){CF.toast(txt(errors.publishChanged));return;}amount=D.current(p)?String(D.current(p).amount):'';publishInitial=publishSnapshot();open('publish');}
  async function createProject(){if(busy)return;error='';
    const cfg=D.type(kind);
    ...
    const r=Number(rate);
    if(!(r>0&&r<=cfg.maxRate*100)){error=L('Enter a pledge rate above 0 and no more than '+(cfg.maxRate*100)+'%.','请输入大于 0 且不超过 '+(cfg.maxRate*100)+'% 的质押率。');refocus='ls-rate';CF.render();return;}
    const t=Number(term);
    if(!Number.isInteger(t)||t<3||t>cfg.maxTerm){error=L('Enter a whole number of months between 3 and '+cfg.maxTerm+'.','请输入 3～'+cfg.maxTerm+' 之间的整月数。');refocus='ls-term';CF.render();return;}
    if(!settle.length){error=L('Choose at least one reference settlement currency.','请至少选择一种参考结算币种。');CF.render();return;}
    busy=true;CF.render();await new Promise(x=>setTimeout(x,350));
    try{
      const p=D.createProject({name,kind,rate,term,settle},editing?D.project(editing):null);
      name='';kind='';rate='';term='';settle=[];editing=null;goto(p.id);
      CF.toast(L('Submitted for review. The contract is deployed once it is approved.','已提交审核，通过后由平台部署本项目质押合约。'));
  ...
  async function commitApplication(){if(busy)return;busy=true;CF.render();await new Promise(r=>setTimeout(r,450));try{if(submitFail)throw Error('submitFailed');D.execute(project(),'deposit',selection);selection=[];reviewReturn=false;S.layer=null;CF.toast(L('Signed and sent. Each token counts towards the pending review amount once it lands.','已发起上链，成功后先计入待审质押额，等逐笔审核。'));}catch(ex){showError(ex);}finally{busy=false;CF.render();}}
  const layers={
    ...
    review:()=>({title:L('Pledge review','质押审核'),html:'<div class="ls-review-surface ls-stack">'+applications(project())+'</div>',foot:(D.mine(project())?'<span class="ls-review-selected">'+L('Selected: ','已选：')+selection.length+L(' tokens',' 张')+'</span>':'')+btn('closelayer',L('Close','关闭'))+(D.mine(project())?btn('ls-release',L('Release selected','解押取回所选'),'',true,!selection.length):'')}),
    ...
    pledge:()=>({title:pledgeLabel(project()),html:'<div class="ls-wide ls-stack">'+chainNotice()+tokenTable(D.candidates(project()),'select','select')+selectionSummary()+err()+'</div>',foot:btn('closelayer',L('Cancel','取消'))+btn('ls-confirm-application',L('Sign and send on chain','签名并发起上链'),'',true,!selection.length)}),
    application:()=>({title:L('Confirm and sign','核对并签名'),html:'<div class="ls-medium ls-stack">'+chainNotice()+selectionSummary()+err()+'</div>',foot:btn('ls-application-back',L('Back','返回'),'',false,busy)+btn('ls-submit',busy?L('Submitting…','提交中…'):L('Confirm submission','确认提交'),'',true,busy)}),
    publish:()=>{const p=project(),d=D.current(p),max=D.numbers(p).free+(d?d.amount:0);return {title:d?L('Edit financing amount','修改融资需求金额'):L('Publish financing request','发布融资需求'),html:'<div class="ls-medium ls-stack"><p>'+L('Available amount: ','当前可融金额：')+'<b class="num">'+usd(max)+'</b></p>'+field('ls-amount','Financing request (USD) *','融资需求金额（USD）*',amount,'text','inputmode="decimal" aria-describedby="ls-amount-error" aria-invalid="'+!!error+'"')+'<div class="field"><p class="field-label" id="ls-settle-label">'+L('Reference settlement currency','参考结算币种')+'</p><p class="field-static" role="note" aria-labelledby="ls-settle-label">'+E((p.settle||[]).join(' / '))+(p.settle?.length?'':L('Not provided','未提供'))+'</p><p class="hint">'+L('Set when the project was created and locked to it. For reference only; funders may quote in another supported currency.','随项目创建时确定并锁定，仅供参考，不限制资金方实际报价币种。')+'</p></div>'+'<div id="ls-amount-error">'+err()+'</div>'+note(L('Publishing makes this request public and reserves the amount. The project term is one year from first publication.','发布后融资需求将公开并占用额度，项目有效期为首次发布起一年。'))+'</div>',foot:btn('closelayer',L('Cancel','取消'))+btn('ls-publish-confirm',L('Confirm','确认'),'',true,busy)};},
    release:()=>{const p=project(),n=D.numbers(p);
      const all=D.tokens.filter(t=>t.pool===p.id&&t.pledge==='pledged'||t.releasedFrom===p.id&&t.pledge==='released');
      const stateOf=t=>t.pledge==='released'?'released':D.appOf(p,t)?.state;
      const chosen=selection.map(D.token).filter(t=>t.pledge==='pledged'&&t.valid&&stateOf(t)==='approved').reduce((v,t)=>v+t.value,0);
      const group=(title,note2,rows,key)=>rows.length?'<section><h3>'+title+'</h3>'+(note2||'')+tokenTable(rows,'release',key)+'</section>':'';
      /* 五类分别呈现：只有已通过审核的有效抵押物受可撤回上限约束。 */
      const html='<div class="ls-wide ls-stack">'
        +group(L('Awaiting review · withdraw any time','待审代币 · 可随时撤回'),
               note(L('These tokens are in the contract and counted towards the pending review amount. Withdrawing ends this review.','这些代币在合约内并计入待审质押额；撤回后本笔审核终结为已撤回。')),
               all.filter(t=>stateOf(t)==='review'),'waiting')
        +group(L('Rejected · release to get them back','已驳回 · 需解押取回'),
               note(L('Rejected tokens stay in the contract and keep counting towards the pending review amount until you release them.','驳回后代币仍在合约内并继续计入待审质押额，须自助解押取回后才能再次质押。'),'warn'),
               all.filter(t=>stateOf(t)==='rejected'),'rejected')
        +group(L('Expired · withdraw at any time','已失效 · 可随时撤回'),'',
               all.filter(t=>!t.valid&&t.pledge==='pledged'&&stateOf(t)==='approved'),'invalid')
        +group(L('Valid collateral · subject to limit','有效抵押物 · 受额度限制'),
               note(L('Current withdrawal limit: ','当前最多可撤回：')+usd(n.withdraw)),
               all.filter(t=>t.valid&&t.pledge==='pledged'&&stateOf(t)==='approved'),'valid')
        +(all.some(t=>t.valid&&t.pledge==='pledged'&&stateOf(t)==='approved')?'<p role="status">'+L('Selected valid collateral: ','拟撤回有效抵押物价值：')+usd(chosen)+L(' · Difference: ',' · 超出差额：')+usd(Math.max(0,chosen-n.withdraw))+'</p>'+(chosen>n.withdraw?note(txt(errors.withdrawLimit),'danger'):''):'')
        +group(L('Released · awaiting collection','已释放 · 待提取'),'',all.filter(t=>t.pledge==='released'),'released')
        +note(L('Tokens are returned to their original holding address. You sign each release yourself; gas is paid by your company and is not refunded if it fails.','代币将转回质押前的原持有地址，不能更改目的地址；解押由你本人签名，gas 自担且失败不退。'))
        +selectionSummary()+err()+'</div>';
      return {title:L('Release collateral','解除质押'),html,foot:btn('closelayer',L('Cancel','取消'))+btn('ls-release-fees',L('Continue','继续'),'',true,!selection.length)};},
  ...
  [LIST,DETAIL,NEW].forEach(id=>CF.review.register(id, {
    ...
    route:()=>id===LIST?'/marketplace':id===NEW?'/project/new':'/project/'+D.projects.find(p=>!['review','returned','available'].includes(p.state)).id,
  ...
  CF.define(CF.LSView={id:'lending-marketplace',pages:[LIST,DETAIL,NEW],breadcrumbRoute:id=>id===LIST?CF.ENTRY[LIST]:null,dict:{en:{navHome:'Home',navAssets:'Asset marketplace',navPlaza:'Lending marketplace',navConsole:'My console',lsDetail:'Financing project',lsNew:'Create project'},zh:{navHome:'首页',navAssets:'资产广场',navPlaza:'借贷广场',navConsole:'我的控制台',lsDetail:'融资项目详情',lsNew:'创建融资项目'}},layers:Object.assign(layers,Q?Q.layers:{}),beforeRender:saveDetail,afterRender:restoreDetail,onBeforeAct(act,v,e){if(Q&&Q.beforeAct(act,e))return true;if(act==='closelayer'&&ownLayer()){if(busy)return true;if(S.layer.key==='discard'){S.layer=discardLayer.layer;flowRestore=discardLayer;discardLayer=null;return true;}requestClose();return true;}return false;},content(id){D.sweep();if(Q)Q.sweep();setTimeout(enhance,0);if(id===NEW)return newPage();if(id===DETAIL)return detailPage();return listPage();},onAct(act,v){
    ...
    try{
      ...
      if(act==='ls-new'){if(S.role!=='asset')throw Error('permission');selection=[];name='';kind='';rate='';term='';settle=[];editing=null;error='';pages.select=1;location.hash='#/project/new';return true;}
      if(act==='ls-amend'){const t=D.project(v);if(!t||!D.mine(t))throw Error('permission');
        editing=t.id;name=txt(t.name);kind=t.kind;rate=String(Math.round(t.rate*1000)/10);term=String(t.term);settle=[...t.settle];error='';
        location.hash='#/project/new';return true;}
      ...
      if(act==='ls-publish-confirm'){const max=D.numbers(p).free+(D.current(p)?.amount||0),n=Number(amount);if(!/^\d+(\.\d{1,2})?$/.test(amount)||n<=0||n>max){error=txt(errors.amount)+' '+L('Available: ','可融金额：')+usd(max)+L(' · Excess: ',' · 超出差额：')+usd(Math.max(0,n-max)||0);refocus='ls-amount';return true;}D.publish(p,n,!!D.current(p),p.settle);closeFlow();pages.demands=1;locateRecord('ls-applications');CF.toast(L('Financing request saved.','融资需求已保存。'));return true;}
      if(act==='ls-release'){openRelease();return true;}
      if(act==='ls-release-fees'){const n=D.numbers(p),total=selection.map(D.token).filter(t=>t.valid&&t.pledge==='pledged'&&D.appOf(p,t)?.state==='approved').reduce((v,t)=>v+t.value,0);if(total>n.withdraw)throw Error('withdrawLimit');operation='release';open('fees');return true;}
      ...
      if(act==='ls-demo-project'){
        try{D.decideProject(p,v,v==='return'?['The pledge rate needs to match the agreed terms.','项目质押率需与约定条款一致，请修改后重新提交。']:null);
          CF.toast(v==='approve'?L('Approved. Pledge contract deployed.','已通过，本项目质押合约已部署。')
            :v==='deployFailed'?L('Deployment failed. Operations will start it again.','部署失败，由管理人员再次发起。')
            :L('Returned to the asset holder.','已退回资产方。'));}
        catch(ex){showError(ex);}return true;}
  ...
  document.addEventListener('input',e=>{if(e.target.id==='ls-name')name=e.target.value;if(e.target.id==='ls-rate')rate=e.target.value;if(e.target.id==='ls-term')term=e.target.value;if(e.target.id==='ls-amount')amount=e.target.value;});
  document.addEventListener('change',e=>{
    ...
    if(el.dataset.settle){const c=el.dataset.settle;settle=el.checked?[...new Set([...settle,c])]:settle.filter(x=>x!==c);}
    if(el.dataset.depositToken){const id=el.dataset.depositToken;if(D.selectable(D.token(id),project()))selection=el.checked?[...new Set([...selection,id])]:selection.filter(x=>x!==id);CF.render();setTimeout(()=>document.querySelector('[data-deposit-token="'+id+'"]')?.focus({preventScroll:true}),0);}
