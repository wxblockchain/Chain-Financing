...
五个既有 HTML 入口（账户、资产广场、借贷广场、我的控制台、消息中心）现在共享 `_shared/portal-entry.js` 的同一依赖清单。任一入口都可通过真实产品导航进入全部面客模块；默认进入资产广场，合法深链优先。源码与单文件导出加载同一组业务脚本，不再以 `sample-pages.js` 的示例页面替代业务模块。
