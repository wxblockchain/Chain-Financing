/* WS-378. Read-only console on the current volumes: no business deadline anywhere, two-step disbursement,
 * company-account receiving snapshots and per-chain transaction identifiers. */
(function(CF){
 'use strict';
 ...
 const labels={valid:['Valid','有效'],void:['Void','失效'],pledged:['Pledged','已质押'],unpledged:['Not pledged','未质押'],draft:['Draft','草稿'],raising:['Fundraising','募集中'],locked:['Locked','已锁定'],financing:['Financing','融资中'],closed:['Closed','已关闭'],settled:['Settled','已结清'],surplus:['Coverage surplus','覆盖有余'],balanced:['Coverage balanced','覆盖持平'],short:['Coverage shortfall','覆盖不足'],effective:['Effective','生效中'],expired:['Expired','已到期'],pending:['Awaiting disbursement','待放款'],confirming:['Awaiting receipt confirmation','待放款确认'],repaying:['Repaying','还款中'],waiting:['Awaiting acceptance','待接受'],rejected:['Rejected','已拒绝'],quoteEnded:['Quote invalidated','报价已失效'],due:['Pending repayment','待还款'],repayConfirming:['Awaiting repayment confirmation','待还款确认'],initial:['Initial plan','初始版'],final:['Final plan','定稿版'],generating:['Plan generating','计划生成中'],open:['Awaiting quotes','待报价'],quoted:['Awaiting quote confirmation','已报价待确认'],funding:['Funding','放款中'],funded:['Disbursed','已放款'],ended:['Invalidated / closed','已失效／已关闭']};
 ...
 const badge=k=>CF.tag(['valid','settled','effective','surplus'].includes(k)?'ok':k==='short'?'warn':['confirming','repayConfirming','waiting','pending','generating'].includes(k)?'accent':'',status(k));
 ...
 /* 本批起控制台没有任何业务时限：等待只如实累计已过去的时长，不做截止时点、剩余时长、倒计时或超期表达。 */
 const elapsed=ms=>{const v=Math.max(0,+ms||0),d=Math.floor(v/DAY),h=Math.floor(v/H);return d>=1?d+L(d>1?' days':' day',' 天'):h>=1?h+L(h>1?' hours':' hour',' 小时'):L('under an hour','不足 1 小时');};
 const since=ms=>ms==null?'—':elapsed(ms);
 /* 交易标识按本笔结算收款链取值；TRON 区块浏览器入口尚未裁定，只给复制与未核验说明，不放死链接。 */
 const CHAINS={ETH:{explorer:h=>'https://etherscan.io/tx/'+h},TRON:{explorer:null}};
 const chainOf=a=>a&&a.kind==='crypto'?a.chain||'':'';
 ...
 function safeRoute(raw,detail=true){
   ...
   const clean=new URLSearchParams();['tab','filter','secondary','project','sort','size','page','y','scroll','focus','id','period','application','query','periodState','targetProject'].forEach(k=>{if(q.has(k))clean.set(k,q.get(k).slice(0,160));});
   if(detail&&q.has('list')){const list=safeRoute(q.get('list'),false);if(list)clean.set('list',list);}
   return '#'+p+(clean.size?'?'+clean:'');
 }
 function view(){const q=params(),tab=tabs().includes(q.get('tab'))?q.get('tab'):tabs()[0];return {tab,filter:q.get('filter')||'',secondary:q.get('secondary')||'',project:q.get('project')||'',query:q.get('query')||'',periodState:q.get('periodState')||'',sort:q.get('sort')||(tab==='credits'?'available-asc':'newest'),size:[20,50,100].includes(+q.get('size'))?+q.get('size'):20,page:Math.min(100000,Math.max(1,parseInt(q.get('page'))||1))};}
 ...
 function normalized(k){
   ...
   if(k==='projects')return S.role==='asset'?ownProjects.map(p=>({...p,project:p.id,status:p.state,n:D.numbers(p),amount:D.current(p)?.amount??null,at:p.created||p.published})):[];
   ...
   if(k==='repayments')return related.filter(q=>q.l7?.record&&['confirmed','settled'].includes(q.l7.state)).map(q=>{
     const deal=q.l7,plan=q.l8,periods=(plan?.periods||[]).map(p=>({...p,status:p.state==='pending'?'repayConfirming':p.state==='settled'?'settled':'due',overdue:p.record?p.record.overdue:Math.max(0,Math.floor((Date.parse(new Date(now()).toISOString().slice(0,10))-Date.parse(p.due))/DAY)),waiting:p.state==='pending'&&p.record?now()-Date.parse(p.record.at):null}));
     ...
     return {id:q.id,business:q.id,project:q.project,request:q.demand,q,deal,plan,periods,current,pending,status:plan?.settledAt?'settled':'repaying',planState:plan?'final':'generating',due:current?.due,at:deal.confirmedAt,overdue:periods.some(p=>p.state==='due'&&p.overdue>0),amount:q.amount};
   });
   return related.filter(q=>k==='quotes'||q.l7).map(q=>{const x=q.l7;return {...q,q,deal:x,status:x?({pending:'pending',waiting:'confirming',confirmed:q.l8?.settledAt?'settled':'repaying',settled:'settled'}[x.state]||'repaying'):({waiting:'waiting',rejected:'rejected',expired:'quoteEnded'}[q.state]||q.state),fdState:x?({pending:'pending',waiting:'confirming'}[x.state]||'funded'):null,request:q.demand,acceptedAt:q.done||null,at:k==='loans'?q.done:q.at,waiting:x?.state==='waiting'&&x.record?now()-Date.parse(x.record.at):null,locked:Math.max(0,(q.state==='waiting'?now():q.done||now())-q.at)};});
 }
 ...
 function matches(r,v){
   ...
   return (!v.filter||picks(v.filter).some(f=>f==='generating'?r.planState==='generating':(v.tab==='loans'?r.fdState:r.status)===f))&&
     ...
     within(v.secondary,v.tab==='tokens'?(r.pledged?'pledged':'unpledged'):v.tab==='projects'?r.n.grade:repayment?(r.overdue?'yes':'no'):'');
 }
 function filtered(){const v=view();let rows=S.st==='empty'?[]:normalized(v.tab);rows=rows.filter(r=>matches(r,v));
   ...
   const value=r=>v.sort.startsWith('value')?r.value:v.sort.startsWith('available')?r.available:v.sort.startsWith('due')?(r.due?Date.parse(r.due):Infinity):Date.parse(r.at)||Number(r.at)||0;
 ...
 function stat(title,value,note=''){return '<div class="s"><div class="k">'+E(title)+'</div><div class="v">'+value+'</div>'+(note?'<div class="n">'+note+'</div>':'')+'</div>';}
 function summary(){
   ...
   /* 代币与授信合并成一条统计条：同一层信息不再拆成卡片盒子。 */
   let assets='';
   if(S.role==='asset'&&(tokenError||!Array.isArray(D.tokens)))assets='<section class="card mc-assets">'+CF.empty(L('Token summary unavailable','代币统计暂不可用'),reloadHint(),'')+'</section>';
   let holdings='';
   if(S.role==='asset'&&!assets){
     holdings=stat(L('All tokens','全部代币'),tokens.length+L(' tokens',' 张'),money(sum(tokens,'value')))+
       stat(L('Of which pledged','其中已质押'),pledged.length+L(' tokens',' 张'),money(sum(pledged,'value'))+(voids.length?' · '+L('void ','已失效 ')+voids.length+L(' tokens',' 张')+L(' excluded',' 不计入覆盖'):''));
   }
   return assets+'<div class="stat-strip mc-summary">'+holdings+(statsError?stat(L('Total credit','总授信额度'),'—',E(reloadHint())):stat(L('Total credit','总授信额度'),money(sum(credits,'total'))))+stat(L('Financed balance','已融额度'),money(sum(credits,'principal')))+stat(L('Available credit','剩余可用授信'),money(sum(credits,'available')),L('Subject to assessment when publishing a request.','实际可融金额以发布需求时核定结果为准。'))+'</div>';
 }
 const filterOptions={tokens:['valid','void'],projects:['draft','raising','locked','financing','closed','settled'],credits:['effective','expired'],loans:['pending','confirming','funded'],repayments:['repaying','settled','generating'],quotes:['waiting','rejected','quoteEnded','pending','confirming','repaying','settled']};
 ...
 function filters(){const v=view(),all=['',L('All','全部')];let h=pick('filter',v.tab==='tokens'?L('Token status','代币状态'):v.tab==='projects'?L('Project status','项目状态'):L('Status','状态'),[all,...filterOptions[v.tab].map(k=>[k,status(k)])],v.filter);
   const sec=v.tab==='tokens'?['pledged','unpledged']:v.tab==='projects'?['surplus','balanced','short']:v.tab==='repayments'?['yes','no']:[];
   if(sec.length)h+=pick('secondary',v.tab==='tokens'?L('Pledge status','质押状态'):v.tab==='projects'?L('Coverage','质押覆盖状态'):L('Repayment overdue','还款是否逾期'),[all,...sec.map(k=>[k,k==='yes'?L('Yes','是'):k==='no'?L('No','否'):status(k)])],v.secondary);
   if(v.tab==='tokens')h+=pick('project',L('Project','所属融资项目'),[all,...normalized('projects').map(p=>[p.id,text(p.name)])],v.project);
   if(v.tab==='repayments')h+=pick('periodState',L('Contains instalments','包含期次状态'),[all,...['due','repayConfirming','settled'].map(k=>[k,status(k)])],v.periodState);
   ...
   return '<div class="filterbar">'+h+'<div class="fb-acts">'+(['loans','repayments'].includes(v.tab)?b('search',L('Search','查询')):'')+b('clear',L('Reset filters','清空筛选'))+'</div></div>'+(Object.keys(v).some(k=>['filter','secondary','project','query','periodState'].includes(k)&&v[k])?'<div class="mc-filter-note" role="status">'+L('Filtered by: ','已按以下条件过滤：')+E([picks(v.filter).map(status).join(' / '),picks(v.secondary).map(k=>k==='yes'?L('Overdue','逾期'):k==='no'?L('Not overdue','未逾期'):status(k)).join(' / '),picks(v.periodState).map(status).join(' / '),v.query,picks(v.project).join(' / ')].filter(Boolean).join(' · '))+'</div>':'');
 }
 function columns(k){return {
   ...
   quotes:[[L('Business / application','融资业务／融资申请编号'),'id'],[L('Project','所属项目'),''],[L('Asset holder','资产方'),''],[L('Amount (USD)','融资金额（USD）'),'num'],[L('Status','业务状态'),''],[L('Time locked','已锁定时长'),'']],
 ...
 function rowValues(k,r){const object=t=>'<a class="mc-object" href="'+E(detailHref(k,r.id))+'" data-act="mc-detail" data-v="'+k+'|'+E(r.id)+'">'+E(t)+'</a>';
   ...
   if(k==='repayments')return [object(r.request)+small(E(fund(r.q.fund))),r.current?'<strong>'+money(r.current.settlement,r.q.ccy)+'</strong>'+small(L('Instalment ','第 ')+r.current.seq+L('',' 期')):r.plan?L('No instalments awaiting repayment','暂无待还期次'):'—',r.current?date(r.current.due)+(r.current.overdue?small(L('Overdue ','逾期 ')+r.current.overdue+L(' days',' 天')):now()<r.current.open?small(L('Opens ','开启于 ')+date(r.current.open)):''):'—',badge(r.status)+small(status(r.planState)),r.pending.length?badge('repayConfirming')+small(r.pending.map(p=>L('Instalment ','第 ')+p.seq+L('',' 期')+L(' · submitted ',' · 已提交 ')+elapsed(p.waiting)).join(' / ')):L('No pending confirmations','暂无待确认'),time(r.at)];
   const state=badge(k==='loans'?r.fdState:r.status);
   if(k==='quotes')return [object(r.id)+small(E(r.request)),E(text(D.project(r.project)?.name)),E(owner(r.owner)),CF.fmtAmt(r.amount),state,elapsed(r.locked)+small(r.status==='waiting'?L('Still awaiting the asset holder','仍在等待资产方处理'):L('Stopped when the quote was handled','处理后停止累计'))];
   return [object(r.request)+small(E(r.id)),E(counter(r)),CF.fmtAmt(r.amount),money(r.settlement,r.ccy)+small(r.deal?.record?L('Submitted','已登记'):L('Agreed amount','约定金额')),state+small(r.fdState==='pending'?L('Funder','资金方'):r.fdState==='confirming'?L('Asset holder','资产方'):L('No current action','无当前办理'))+(r.fdState==='confirming'?small(L('Submitted ','已提交 ')+elapsed(r.waiting)):''),time(r.acceptedAt)];
 }
 function table(){const v=view(),all=filtered(),count=Math.max(1,Math.ceil(all.length/v.size));if(v.page>count&&S.st!=='error'&&S.st!=='loading'&&!tokenError){v.page=count;const q=params();q.set('page',count);history.replaceState(null,'','#'+path()+'?'+q);}const rs=all.slice((v.page-1)*v.size,v.page*v.size),cols=columns(v.tab);
   ...
   if(!all.length&&S.st!=='loading'&&S.st!=='noresult'&&!['filter','secondary','project','query','periodState'].some(k=>v[k]))return CF.empty(v.tab==='tokens'?L('No displayable tokens for this company','本企业暂无可展示的代币'):v.tab==='projects'?L('No financing projects yet','尚未创建融资项目'):L('No records yet','暂无记录'),'','');
 ...
 function gate(){if(S.role==='signed')return '<div class="mc-error">'+CF.empty(L('This page is not available for your current role','该页面不适用于当前身份'),L('You can continue exploring public information.','你可以继续浏览公开信息。'),'<button class="btn primary" data-act="go" data-v="/assets">'+L('Back to assets','返回资产广场')+'</button>')+'</div>';
  return '<div class="mc-error">'+CF.empty(L(S.role==='limited'?'Complete account setup':'Sign in to view your console',S.role==='limited'?'请先完成账户必办事项':'登录后查看我的控制台'),L('Your company information is available after sign-in.','登录后可查看本企业数据。'),'<button class="btn primary" data-act="signin">'+L('Sign in','登录')+'</button>')+'</div>';}
 ...
 function account(a,missing){
   ...
   return dl(accountRows(a))+'<p class="mc-small">'+L('Shown masked here; the paying party reads the full account in the lending marketplace.','此处脱敏显示，完整账户由付款当事方在借贷广场查看。')+'</p>';
 }
 function transaction(hash,a){
   if(!hash)return '';
   const chain=chainOf(a),spec=CHAINS[chain];
   return dl([[L('Transaction ID','交易标识'),copy(hash)],[L('Settlement chain','结算收款链'),E(chain||'—')]])+
     (spec&&spec.explorer?CF.linkOut(spec.explorer(hash),L('Open transaction in block explorer','在区块浏览器查看交易'))+'<p class="mc-small">'+L('The platform has not verified this transaction. The link is for your own checks.','平台未核验该交易，链接仅供自行查验。')+'</p>'
      :'<p class="mc-small">'+L('The platform has not verified this transaction. Copy the transaction ID and check it on a block explorer you trust.','平台未核验该交易，请复制交易标识自行查验。')+'</p>');
 }
 /* 授信协议为非必填：有则给份数与提交时间，缺失只说明本笔未提供，不显示待上传或待审核。 */
 function agreements(ownerId,fundId){
   const rows=(Q.data().credits||[]).filter(c=>c.owner===ownerId&&c.fund===fundId).flatMap(c=>(c.history||[]).filter(h=>h.agreements?.length).map(h=>({at:h.at,n:h.agreements.length})));
   if(!rows.length)return '<p class="mc-small">'+L('Not provided for this business.','本笔未提供。')+'</p>';
   return dl([[L('Agreement files','协议文件'),rows.reduce((n,h)=>n+h.n,0)+L(' files · read them in the marketplace',' 份 · 到广场查看')],[L('Latest submitted at','最近提交时间'),time(rows.at(-1).at)]]);
 }
 function evidence(record,a){if(!record)return '';const f=record.form||record,files=f.files||[];
   return dl([[L('Supporting files','凭证／补充材料'),files.length?files.length+L(' files · view in marketplace',' 份 · 到广场查看'):L('No files provided','未提供文件')]])+transaction(f.hash,a);
 }
 function periodFacts(r,p){return dl([[L('Instalment ID','期次编号'),copy(p.id)],[L('Principal due','应还本金'),money(p.principal)],[L('Interest due','应还利息'),money(p.interest)],[L('Total due','应还合计'),money(p.total)],[L('Settlement amount','结算金额'),money(p.settlement,r.q.ccy)],[L('Accrual period','计息区间'),date(p.from)+' → '+date(p.due)],[L('Accrual days','计息天数'),p.days],[L('Payment window opens','还款入口开启时间'),time(p.open)],[L('Overdue days','逾期天数'),p.overdue+(p.record?L(' · frozen at submission',' · 提交后冻结'):'')],p.record&&[L('Record ID','还款记录编号'),copy(p.record.id)],p.record&&[L('Repaid at','还款时间'),time(p.record.paidAt)],p.record&&[L('Submitted at','提交时间'),time(p.record.at)],p.state==='pending'&&[L('Awaiting funder confirmation','等待资金方确认'),since(p.waiting)],p.record&&[L('Confirmed at','还款确认时间'),p.record.confirmedAt?time(p.record.confirmedAt):L('Awaiting the funder','待资金方确认')],p.record?.account&&[L('Receiving account','收款账户'),Q.accountName(p.record.account)+'<br>'+Q.accountKey(p.record.account)],p.record&&[L('Repayment nature','还款性质'),p.record.overdue?L('Overdue repayment','逾期还款'):L('Normal repayment','正常还款')],p.record&&[L('Note','备注'),E(p.record.note||'—')]])+evidence(p.record,p.record?.account);}
 function repaymentSections(r){
   ...
   if(planError){add('mc-plan',L('Repayment schedule','还款计划'),CF.empty(L('Repayment schedule unavailable','还款计划暂不可用'),L('Receipt confirmation is preserved. ','到账确认结果已保留。')+reloadHint(),''));}
     ...
     add('mc-pending',L('Awaiting confirmation','待还款确认'),r.pending.length?r.pending.map(p=>'<article class="mc-record"><div class="mc-record-head"><b>'+L('Instalment ','第 ')+p.seq+L('',' 期')+'</b>'+badge(p.status)+'</div>'+dl([[L('Instalment / record','期次／还款记录'),copy(p.id)+'<br>'+copy(p.record.id)],[L('Submitted at','提交时间'),time(p.record.at)],[L('Awaiting funder confirmation','等待资金方确认'),since(p.waiting)],[L('Overdue days','逾期天数'),p.overdue?p.overdue+L(' days · frozen at submission',' 天 · 提交后冻结'):'0'],[L('Settlement amount','结算金额'),money(p.settlement,r.q.ccy)]])+'</article>').join('')+CF.note('',L('Confirmation has no deadline. Overdue days stay frozen, the platform never confirms on your behalf, and confirming later gives the same result.','确认没有时限：逾期天数保持冻结，平台不会自动或代为确认，任何时点确认的结果一致。')):CF.empty(L('No pending confirmations','暂无待确认记录'),''));
   ...
   add('mc-account',L('Funder’s repayment receiving account','资金方还款收款账户'),account(r.deal.record.account,L('Awaiting submission by the funder','待资金方提交'))+'<p class="mc-small">'+L('This account receives the repayments of this business.','该账户用于收取本笔业务的还款。')+'</p>');
   return out;
 }
 function details(k,r){
   ...
   if(k==='tokens'){
     ...
     const n=r.n;
     add('mc-basic',L('Project overview','项目概况'),dl([[L('Project ID','融资项目编号'),copy(r.id)],[L('Project name','融资项目名称'),E(text(r.name))],[L('Token type','代币类型'),E(text(D.type(r.kind).label))],[L('Project status','项目状态'),badge(r.status)],[L('Project deadline','项目有效截止日'),date(r.expires)],[L('Project term','项目期限'),r.term+L(' months',' 个月')],[L('Created at','创建时间'),time(r.created)],[L('First published','首次发布时间'),time(r.published)],[L('Pledge contract address','质押合约地址'),r.contract?copy(r.contract):'—'],[L('Repayment type','还款方式'),E(text(r.repay))],[L('Reference settlement currency','参考结算币种'),E((r.settle||[]).join(' / '))||'—'],[L('SPV','SPV 机构名称'),E(text(r.spv))],r.closeReason&&[L('Close reason','关闭原因'),E(r.closeReason)]]));
     add('mc-coverage',L('Collateral and coverage','质押与覆盖'),dl([[L('Total pledged value','总质押额'),money(n.total)],[L('Pending review value','待审质押额'),money(n.pendingValue)],[L('Eligible pledge amount','有效质押额'),money(n.limit)],[L('Current request','本轮融资需求'),D.current(r)?money(r.amount):L('No active request','暂无当前需求')],[L('Outstanding financing','已融资余额'),money(r.balance)],[L('Project pledge rate','项目质押率'),Math.round(n.rate*1000)/10+'%'],[L('Eligible collateral value','有效质押价值'),money(n.value)+' · '+n.valid.length+L(' tokens',' 张')],[L('Tokens inside the contract','合约内代币张数'),n.pool.length],[L('Coverage status','质押覆盖状态'),badge(n.grade)],[L('Coverage shortfall','覆盖缺口'),money(n.gap)],[L('Additional asset value needed','需追加资产价值'),money(n.rate?n.gap/n.rate:0)],[L('Void pledged tokens','已质押失效代币'),n.pool.filter(t=>!t.valid).length],[L('Withdrawable token value','可提取代币价值'),r.l8ReleasePending?L('Release result pending','释放结果待取得'):money(D.tokens.filter(t=>t.releasedFrom===r.id&&t.pledge==='released').reduce((n,t)=>n+t.value,0)+n.withdraw)]]));
     add('mc-rounds',L('Financing applications','融资申请信息'),r.demands.length?[...r.demands].sort((a,b)=>Date.parse(b.at)-Date.parse(a.at)).map(d=>{
       ...
       const quoteRow=q=>'<article class="mc-record"><div class="mc-record-head"><b>'+E(fund(q.fund))+'</b>'+badge(({expired:'quoteEnded',funding:'pending',funded:'repaying'})[q.state]||q.state)+'</div>'+dl([[L('Business ID','融资业务编号'),copy(q.id)],[L('Annual rate','年化利率'),E(q.rate)+'%'],[L('Quoted at','报价时间'),time(q.at)],[L('Reason','终结原因'),E(q.reason||'—')]])+link('detail',L('View quote details','查看报价详情'),'quotes|'+q.id)+(q.l7?link('detail',L('View disbursement details','查看放款详情'),'loans|'+q.id):'')+'</article>';
     ...
     if(k==='quotes')add('mc-quote',L('Quote and lock','报价与锁定'),dl([[L('Business status','业务状态'),badge(r.status)],[L('Quote submitted','报价提交时间'),time(r.at)],[L('Lock starts','锁定开始时间'),time(r.at)],[L('Locked by','锁定机构'),E(fund(r.fund))],[L('Time locked','已锁定时长'),elapsed(r.locked)],[L('Result time','处理时间'),r.done?time(r.done):'—'],[L('Project coverage','当前项目覆盖'),badge(D.numbers(D.project(r.project)).grade)],[L('End reason','失效原因'),E(r.reason||'—')]])+(r.status==='waiting'?CF.note('',L('A submitted quote has no validity period and cannot be withdrawn, amended or extended. It ends only when the asset holder accepts or rejects it, or when the financing request itself ends.','报价提交后没有有效期，也不能撤回、修改或展期；只有资产方接受、拒绝，或所属融资需求终结才结束。')):''));
     const x=r.deal;
     if(x){if(k==='loans')add('mc-agreement',L('Credit agreement','授信协议'),agreements(x.owner,x.fund));
       add('mc-record',L('Disbursement record','放款记录'),x.record?dl([[L('Record ID','放款记录编号'),copy(x.record.id)],[L('Disbursement amount','放款金额'),money(x.settlement,x.ccy)],[L('Disbursed at','发放时间'),time(x.record.form.paidAt)+small(L('Entered by the funder, for business reference only','由资金方填写，仅作业务参考'))],[L('Submitted at','提交时间'),time(x.record.at)+small(L('Recorded by the platform; interest starts on this UTC date','平台记录；起息日取该 UTC 日期'))],[L('Receipt confirmed','到账确认时间'),x.confirmedAt?time(x.confirmedAt):L('Not confirmed','未确认')],x.state==='waiting'&&[L('Awaiting asset holder confirmation','等待资产方确认'),since(r.waiting)],[L('Received amount','到账实收金额'),x.record.received?money(x.record.received,x.ccy):L('Not provided','未提供')],[L('Note','放款备注'),E(x.record.form.note||'—')]])+(x.state==='waiting'?CF.note('',L('Receipt confirmation has no deadline; the two parties agree the timing offline. Until it is confirmed the repayment schedule is not generated, both in-flight amounts stay occupied and the collateral is not released.','到账确认没有时限，时点由双方线下商定。确认前还款计划不生成，两类在途金额继续占用，质押不释放。')):''):CF.empty(L('No disbursement record yet','暂无放款记录'),L('The funder has not submitted a record.','资金方尚未提交放款记录。')));
       add('mc-asset-account',L('Asset holder’s disbursement receiving account','资产方放款收款账户'),account(x.account,L('No receiving account on record','暂无收款账户记录')));
       add('mc-account',L('Funder’s repayment receiving account','资金方还款收款账户'),account(x.record?.account,L('Awaiting submission by the funder','待资金方提交'))+'<p class="mc-small">'+L('This account receives the future repayments of this business.','该账户用于收取本笔业务未来的还款。')+'</p>');
       if(x.record)add('mc-evidence',L('Payment evidence','放款凭证摘要'),evidence(x.record,x.account));
 ...
 function detail(k){
   ...
   const second=k==='repayments'?stat(L('Disbursed amount','放款金额'),money(r.deal.settlement,r.q.ccy)):k==='projects'?stat(L('Eligible pledge amount','有效质押额'),money(r.n.limit)):k==='credits'?stat(L('Credit limit','授信额度'),money(r.total)):k==='tokens'?stat(L('Receivable due','应收账款到期日'),date(r.due)):stat(r.deal?.record?L('Disbursed amount','放款金额'):L('Agreed disbursement amount','约定放款金额'),money(r.settlement,r.ccy));
   return '<div class="mc-root mc-detail"><div class="page-head"><div><h1 class="page-title" tabindex="-1">'+E(text(titles[k]))+'</h1><div class="mc-detail-meta">'+copy(k==='repayments'?r.request:r.id)+badge(k==='loans'?r.fdState:r.status)+'</div></div><div class="mc-quick-actions" aria-label="'+L('Project navigation','项目导航')+'">'+controls(k,r)+'</div></div>'+(detailState==='stale'?CF.note('warn',L('The business state changed. The previous action is no longer available.','业务状态已变化，原操作已不可用。')):'')+'<div class="stat-strip mc-detail-metrics">'+stat(metric,money(amount))+second+'</div><nav class="mc-chapters" aria-label="'+L('On this page','本页章节')+'">'+sections.map((x,i)=>'<button type="button" data-act="mc-section" data-v="'+x.id+'"'+(!i?' aria-current="location"':'')+'>'+E(x.title)+'</button>').join('')+'</nav><div class="detail-stack">'+sections.map(x=>x.html).join('')+'</div></div>';
 }
 ...
 const mod={...old,id:'my-console-v13',pages:['P-MC-01',...kinds.map((_,i)=>'P-MC-0'+(i+2))],dict,
   ...
   onAct(a,v,e){
     ...
     if(key==='tab'){returnNotice=false;setView({tab:v,filter:'',secondary:'',project:'',query:'',periodState:'',sort:v==='credits'?'available-asc':'newest',page:1});}
     if(key==='clear'){S.st='default';returnNotice=false;S.menu=null;setView({filter:'',secondary:'',project:'',query:'',periodState:'',page:1});}
 ...
 document.addEventListener('change',e=>{
   ...
   if(e.target.id==='mc-targetProject'&&path().startsWith('/console/')){const q=params();q.set('targetProject',e.target.value);history.replaceState(null,'','#'+path()+'?'+q);CF.render();document.getElementById('mc-targetProject')?.focus();return;}if(path()!=='/console'||!e.target.id.startsWith('mc-'))return;const k=e.target.id.slice(3);if(['filter','secondary','project','sort','size','periodState'].includes(k))setView({[k]:k==='size'?+e.target.value:e.target.value,page:1});});
