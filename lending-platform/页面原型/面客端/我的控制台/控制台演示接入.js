...
(function(CF){
  ...
  const account={id:'EA-DEMO-F01',kind:'fiat',ccy:'USD',label:['Singapore operating account','新加坡收款账户'],iban:'SG64DEMO0000000001',bank:'Demo Bank Limited',swift:'DEMOSGSG',country:'SG',bankCountry:'SG',addressCountry:'SG',addressCity:'Singapore',addressLine:'18 Example Avenue, #08-01',bankCity:'Singapore'};
  const cryptoAccount={id:'EA-DEMO-C01',kind:'crypto',chain:'ETH',label:['ETH receiving address','ETH 收款地址'],address:'0x00000000000000000000000000000000000000a1'};
  ...
  samples.forEach(([state,ccy],i)=>{
    ...
    const q={id:business,project:id,owner:p.owner,demand:request,amount,ccy,rate:'6.40',settlement:amount,fx:{value:1,version:'FX-MC-001',at:start,source:['Demonstration snapshot','演示汇率快照']},at:start,state:confirmed?'funded':state==='terminated'?'terminated':'funding',fund:'fund-a',repay:iso(end),done:start+H,progress:{selected:ccy==='USD'?account.id:cryptoAccount.id,account:{...(ccy==='USD'?account:cryptoAccount)},step:2,confirmed:true,viewed:true}};
