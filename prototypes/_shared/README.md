...
三个模块原型共用的 token、组件、页面登记表与运行时。**这里是唯一来源**：任何一处改动对所有模块同时生效。
...
- 点铃铛或「消息中心」落到 `CF.MSG_PAGE` 指定的页面（资产端 `P-A18`、管理端 `P-M20`）。
  这两页由 `prototypes/消息通知/` 交付，不在本文件里时按跨文件规则**真的跳过去**。
