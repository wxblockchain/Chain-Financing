...
- 铃铛只表现「有未读角标 / 无未读角标」两种样子，**不实现计数**——形态与行为归 WS-304。
- 点铃铛或「消息中心」进入 `CF.MSG_PAGE` 指定的页面（资产端 `P-A18`、管理端 `P-M20`）。
  这两页登记在 `CF.EXTERNAL` 里：**它们不属于本仓库的任何原型文件**，由 shell 渲染成
  标注归属的入口占位页，并带一个切换角标状态的状态条。
- 外部占位页不进模块自定义的 URL 方案（模块不认识这些 ID），统一走默认 `#/<page-id>`，
  这样深链与刷新都能停在占位页而不是掉回模块首页。

## 侧栏是一份，文件是多份

两份原型各自是独立可打开的文件，但侧栏菜单来自同一张 `CF.NAV`，所以两边渲染出的菜单完全一致。
点到不属于当前文件的菜单项时，shell 会按 `CF.MODULES` + `CF.ENTRY` 生成相对地址，
**直接跳到对方原型文件的对应页面**（例如从协议管理点「账户设置」→ `../账户与登录/…#/p-m05`），
而不是停在占位页。顶栏账户菜单里的「账户设置」同理。

这要求两个模块目录是同级的，且都在本地存在——从仓库直接打开或整个 `prototypes/` 一起拷走都满足。
