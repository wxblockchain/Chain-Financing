...
金融服务端登录（WS-311）是本平台第一个**面客画布**模块，登记为 `end = asset`，与运营端的 `end = admin` 共存于同一张登记表；两者的差异只落在 `base.css` 的 `[data-end="asset"]` 密度覆盖上，不是两套色板。它的侧栏第二项（账户设置）随会话身份在 `P-F04`（资产方）/ `P-F24`（资金方）之间解析，游客态两条都不出现——这是权限矩阵第 16 行在导航上的落点，由模块更新 `CF.NAV.asset` 实现，菜单仍由公共层按登记表渲染，模块不自建导航。
