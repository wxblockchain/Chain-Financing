...
| 借贷广场 · 融资需求与代币质押（WS-324） | 金融服务端（**面客 portal 顶栏骨架**） | [v1.0-借贷广场-融资需求与代币质押-原型.html](借贷广场-融资需求与代币质押/v1.0-借贷广场-融资需求与代币质押-原型.html) | [交互与覆盖说明](借贷广场-融资需求与代币质押/README.md)；P-LS-01 融资需求广场 / P-LS-02 融资需求详情（左右两栏 + 池内资产变动与融资变动两张图）/ P-LS-03 建池与发布两段式 / P-LS-04 我的融资项目（原型侧暂定号）；游客 · 资产方 · 资金方三身份由顶栏上下文操作区切换，链上与后端为模拟。继承 4.3 顶栏母版（金融服务端登录），token / 组件 / 运行时取 `_shared`，页面登记在本平台 `registry.js` |
