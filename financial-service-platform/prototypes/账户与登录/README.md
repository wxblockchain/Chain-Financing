...
仓库基线：`wxblockchain/Chain-Financing`，`cly-V1.0.0`。首次入库基于 `ec0bd45`；本轮增量修改基于 `f8614ef`（WS-309 消息通知、WS-310 协议管理两份原型同期已补齐各自的 PRD 摘录面板）。公共视觉与运行时文件（`asset-platform/prototypes/_shared/`）与平台登记表（`../_shared/registry.js`）相对上一轮均未变化。
