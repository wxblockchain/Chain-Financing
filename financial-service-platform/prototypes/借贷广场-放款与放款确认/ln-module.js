...
function stageActions(d, acts){
  ...
  if(!acts.length)
    return '<div class="fa"><p class="hint" style="margin:0">' +
      (S.role === 'other'
        ? L('This deal belongs to another asset owner and funder — there is nothing for you to act on here.',
            '本笔业务属于另一对资产方与资金方，这里没有您可执行的操作。')
        : d.st === 'S-FD-6'
...
function repayFiatFields(){
  ...
  '<p class="hint">' +
  L('All five are required as a group — the correspondent bank included.',
    '五项<b>整组必填</b>，中转行也在内。') + '</p>';
}
function repayOk(d){
...
function disbForm(d){
  ...
  body += '<div style="border-top:1px solid var(--border);margin:16px 0 14px;padding-top:14px">' +
    ...
    L('This is where <b>you</b> will be repaid. <b>It cannot be changed after you submit</b>, so check it now.',
      '这是<b>您将来收还款</b>的账户。<b>提交后本期不可修改</b>，请现在核对。') + '</p>' +
    (isFiat(d)
      ? repayFiatFields()
...
function dispPanel(){
  ...
  return '<div class="u-2nd' + (k === 'stop' ? ' stop' : '') + '"><div class="h">' + E(L(m.h[0], m.h[1])) + '</div>' +
    ...
    L('Reason required, 1–' + REASON_MAX + ' characters. <b>Visible to the asset owner</b>. ' + v.length + ' / ' + REASON_MAX + ' used.',
      '原因必填，1～' + REASON_MAX + ' 字。<b>对资产方可见</b>。已填 ' + v.length + ' / ' + REASON_MAX + ' 字。') + '</p>' +
    '<div class="bar">' +
...
function drawerDisb(){
  ...
  var body =
    ...
    sec(L('2 · Fill in the disbursement record','② 填写放款记录'),
      ...
      L('This step performs <b>no on-chain operation and consumes no gas</b>.',
        '本步骤<b>不产生任何链上操作、不消耗 gas</b>。') + '</p>');
...
function countdown(d, forAsset){
  ...
  return '<div class="cd' + cls + '">' +
    ...
    (k.over ? '<div style="margin-top:10px">' + mark(
        ...
        L('The deal is still awaiting your confirmation and the entry still works.',
          '业务仍待您确认，入口照常可用。')) + '</div>' : '') +
    (k.soon ? '<p class="hint" style="color:var(--warn);margin-top:9px">' +
      L('Under ' + NEAR_HOURS + ' hours left — the countdown is now in minutes.',
        '剩余不足 ' + NEAR_HOURS + ' 小时，倒计时已切到分钟精度。') + '</p>' : '') +
    '</div>';
}
function recordBlock(d, full){
  ...
  var proof = fiat
    ...
    : '<div class="hash"><div class="hv">' + E(ln.hash) + '</div>' +
      ...
      '<p class="hint" style="margin-top:9px">' +
      L('The hash passed a <b>format check only — there was no on-chain verification</b>.',
        '这条哈希<b>只校验过格式，没有做链上核验</b>。') + '</p></div>';
...
function feeBlock(d){
  return '<div class="fee">' +
    ...
    why(L('What if the gap is far bigger than a fee?','差额大到不像手续费怎么办'),
      L('<b>Do not confirm yet.</b> Use the support mailbox below to sort it out offline — confirmation cannot be undone.',
        '<b>先不要点确认</b>，走下面的客服邮箱线下核实——确认之后不可撤销。')) + '</p>';
}
...
var SUBMIT_OUT = [
  ...
  ['cov',    ['Coverage dropped below the invariant at submission','提交瞬间质押覆盖跌破']],
  ['race',   ['A colleague submitted the same disbursement first','并发：同事先一步提交了同一笔放款']],
  ['moved',  ['The deal had already left "awaiting disbursement"','提交时业务已不在待放款']],
  ['upload', ['The receipt failed format / size checks','凭证格式或大小不合规']]
];
var CONFIRM_OUT = [
  ['ok',     ['Confirmed successfully','确认成功']],
  ['settle', ['Settlement failed partway, rolled back as a whole','结算部分失败，整体回滚']],
  ['plan',   ['Repayment module not ready, schedule pending','还款模块未就绪，计划生成中']]
];
function outSelect(id, list, cur){
  return '<p class="hint">' + L('Prototype outcome switch: pick what the server returns at submission.',
