...
| **共用主干**（回跳 RT-1～RT-5、用户类型选择页、权限矩阵持表、受限操作引导弹层、三态模型与接口契约 5.2） | 三条登录链路共同经过的路径 | **规格归属 issue 本轮待定**（原拟上收父 issue，随「父 issue 不出 PRD」失效）。本册**只调用、不实现、不改定义**；本册需要主干提供什么，见 **3.2 外部依赖契约表**；未决项见 **Q-F01**——归属决策已上交父 issue WS-311 |
...
本模块统一用 `-F` 后缀（F = Financial service end），与资产平台的 `D-##` / `N-##` / `P-A##`、运营端的 `-O` 均不重叠。三个子 issue 共用这一套编号空间，**因此必须分段位，否则撞号**。本 PRD 取以下段位：

| 前缀 | 含义 | 本 PRD（WS-314）段位 | 建议留给 |
| --- | --- | --- | --- |
| `P-F##` | 页面 / 界面单元 | **`P-F01`～`P-F09`** | WS-315：`P-F10`～`P-F29`；WS-316：`P-F30`～`P-F49`；主干与后续留位：`P-F50`～ |
| `F-F##` | 功能点 | **`F-F01`～`F-F19`** | WS-315：`F-F20`～`F-F49`；WS-316：`F-F50`～ |
| `D-F##` | 已确认口径 | **`D-F80`～`D-F99`**（简报 V4.0 已占用 `D-F01`～`D-F76`，本 PRD 只**引用**不重定义） | WS-315：`D-F100`～；WS-316：`D-F130`～ |
| `N-F##` | 可调参数 | **`N-F10`～`N-F19`**（简报已占用 `N-F02` / `N-F03`） | WS-315：`N-F20`～；WS-316：`N-F30`～ |
| `US-F##` | 本 PRD 新增用户故事 | **`US-F01`～`US-F09`** | 简报第 4 章的 `US-01`～`US-15` 沿用原号 |
| `AC-F##` | 验收标准 | **`AC-F01`～`AC-F59`** | WS-315：`AC-F60`～；WS-316：`AC-F100`～ |

**沿用不改号的编号**：`SSO-1`～`SSO-5`、`D-F01`～`D-F05`、`D-F70` / `D-F70'`、`D-F71`～`D-F76`、`S-1`～`S-6`、`E-01`～`E-17`、`RT-1`～`RT-5`、`ST-1` / `ST-2`、`X-1` / `X-2`、`M-01`～`M-24`、`K-01`～`K-15`、`N-02`（不做项）、`N-F02` / `N-F03`。

### 0.3 引用路径基准

| 引用写法 | 指向 |
| --- | --- |
| 「简报 6.2」等 | 《需求诊断简报 V4.0 收口版》`financial-service-portal-login-requirements-brief.md`（WS-311 附件，Phase 1 定稿） |
...
> **为什么写成契约表而不是「由某某 issue 定义」**：回跳、类型选择页、权限矩阵、引导弹层、三态契约这五项是三条登录链路共用的主干，**本 issue 不实现、不定义**；但它们的**规格归属 issue 本轮待定**（Q-F01）。归属未定不等于需求可以悬空——下表写的是**本册需要主干提供什么**，谁来写规格是另一回事，**这张表就是本册对主干侧的验收要求**。
...
| **DEP-M4** | **权限矩阵总表与取值语义** | 行清单（能力）、列定义、✅ / ◐ / ⊘ / ❌ 四档语义、D-F70′ 服务端强制总口径 | 本册按册二 8.6 填 `T2 资产方` / `T3 资产方` 两列 | **⊘ 与 ❌ 一律服务端强制**，前端隐藏不构成防线；不得新增第五档取值 |
...
| 编号 | 口径 |
| --- | --- |
| **D-F99** | 上表五项的**规格归属 issue 待定，但实现必须唯一**。本册只调用、不实现；无论 DEP-M1～DEP-M5 最终落到哪个 issue，本册的调用契约不变、无需改写。**三个子 issue 各自实现一套 RT-2 同源白名单校验是明确禁止的**——这是本表里后果最严重的一条（Q-F01） |
...
| **安全（强制条款）** | ① SSO-1～SSO-5 全部服务端实现；② 映射键只能是 `account_id`，服务端拒绝其他实现；③ 主体字段只读，写请求 403（D-F86）；④ 权限矩阵 ⊘ / ❌ 服务端强制（D-F70′）；⑤ 硬卡点服务端实时复核，不凭快照（R-3）；⑥ 跨端失效（S-4）；⑦ **回跳的同源白名单校验必须在主干侧统一实现，本模块不得自建**（3.2 DEP-M1）；⑧ `id_token` / 授权码 / `code_verifier` 不入日志；⑨ 不得以 `Referer` 作为信任依据 |
...
| DEP-5 | 共用主干交付 DEP-M1～DEP-M5 五项 | 本册全部为调用方；**规格归属 issue 待定，见 Q-F01** |
...
| D-F99 | 共用主干五项的规格归属待定，但**实现必须唯一**；三个子 issue 各写一套 RT-2 白名单校验属明确禁止 | 3.2 |
...
> **Q-F02 / Q-F03 / Q-F04 已确认，转为既定口径**（用户 2026-09-10 在本 issue 回复「Q-F02 / Q-F03 / Q-F04；按照建议执行」），正文对应位置的待确认标记已清除，编号 **D-F80 / DEP-3 / RISK-F02 保留不动**。**Q-F01 仍未决，归属决策已上交父 issue WS-311**。
...
| **Q-F01** | **未决 · 已上交父 issue WS-311** | **共用主干五项（DEP-M1～DEP-M5）的规格归属 issue 未定**（原拟上收父 issue，随「父 issue 只出简报、不出 PRD」失效）。本册写成 **3.2 外部依赖契约表**，只调用、不实现，**调用契约与归属无关**，归属定下来后本册无需改写。本册建议：**单开一个「登录主干」issue**——DEP-M5 的 `completeness_level` 由 WS-315（凭据）与 WS-316（申请状态）共同决定，塞给任一方都会让另一方去别人的 issue 里改自己的取值；若一定要塞，整体给 WS-315，另两个引用 | 3.2、5.2、7.4、册二 8.5 / 8.6 |

> **Q-F01 不阻塞本三册的任何内容。** **D-F99** 与 3.2 契约表在归属定下来之前就已把「归属可待定、**实现必须唯一**、三个子 issue 不得各写一套 RT-2 同源白名单校验」锁死——需求不会丢失，开放重定向漏洞也进不来。
